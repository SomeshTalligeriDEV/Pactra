...
> **Source review (17 September 2026):** See the [architecture PDF](../docs/Pactra-System-Architecture-and-User-Workflows.pdf) for the verified implementation and deployment limitations. Examples use placeholder origins; deployment and benchmark artifacts are historical. Windows are tumbling, publication is separate from enforcement, and final-payee matching is unavailable.
...
| | |
|---|---|
| **The argument** | <https://pactra.example> |
| **Documentation** | <https://pactra.example/docs> |
| **Owner's console** | <https://pactra.example/console/> |
| **Look around, no wallet** | <https://pactra.example/console> |
| **A refusal, on chain and published** | <https://pactra.example/refusal/9> |
| **The hostile drill, published whichever way it came out** | <https://pactra.example/drill> |
| **A conduct record, read from the chain** | <https://pactra.example/agent/894130> |
...
**Pactra is asked before the money exists, by a contract with no admin key and
...
|  | A smaller wallet | A per-session cap | Pactra |
|---|---|---|---|
...
The fourth row is the one that decides the others. Under Pactra an agent holds
...
pays it. Pactra does not watch that flow and cannot: money the agent holds
...
| | Agent wallet | Agent wallet behind Pactra |
|---|---|---|
| Who finds the service | the agent | the agent |
| Who holds the money | the agent | the vault; the agent holds no key |
| What pays | the agent signs from its own balance | `pactra_fetch(url)` — the daemon draws exactly the seller's price |
...
Put plainly: the wallet is the agent's hands, and Pactra is the owner's
permission to use them. The settlement rail is the same one; what Pactra adds
...
**What this does not do yet.** Pactra runs on Arc testnet, and marketplace
sellers settle on mainnet chains, so a Pactra mandate cannot pay one of them
...
`attest.pactra.example`, a conduct record at a cent, and
`demo-seller.pactra.example/arc/snapshot`, a live reading of Arc at **$1** — a
...
flowchart TB
    ...
    subgraph daemon[" Daemon — holds the operator key "]
        D["pactraFetch<br/><i>reads the seller's 402, asks the vault,<br/>signs the payment</i>"]
    end
...
| Contract | Address |
|---|---|
| `MandateRegistry` | [`0x5a0a521cd083a3e884dc024c90898589751292b8`](https://testnet.arcscan.app/address/0x5a0a521cd083a3e884dc024c90898589751292b8) |
| `TreeVault` | [`0x6a37c9506740d4d59e343cbbf8d7c2f56aa112eb`](https://testnet.arcscan.app/address/0x6a37c9506740d4d59e343cbbf8d7c2f56aa112eb) |
| `ConductRecord` | [`0xa8bcdda003cddfb672a33e23a0bb375b6bed4a9f`](https://testnet.arcscan.app/address/0xa8bcdda003cddfb672a33e23a0bb375b6bed4a9f) |
<!-- deployed:end -->
...
`0x8004B663056A597Dffe9eCcC1965A193B7388713`. Pactra writes into what is
...
The console runs against the live tree with no wallet: <https://pactra.example/console>.
...
The server is on npm as [`pactra-mcp`](https://www.npmjs.com/package/pactra-mcp),
...
`PACTRA_ENV_FILE` is the key file `init` wrote, as an absolute path — an MCP
...
deployment the package was built against; `PACTRA_VAULT`, `PACTRA_REGISTRY`
and `PACTRA_RECORD` override them. `PACTRA_MCP_NODE` names which node this
...
```bash
git clone https://github.com/SomeshTalligeriDEV/pactra.git && cd pactra
...
Writes `~/.pactra/pactra.env` at `0600` and prints **addresses only**. Send a
little gas to each: an operator that holds a tranche and no gas cannot send the
draw that would earn it.

**2 — sign one mandate.** <https://pactra.example/console/new>, connect your
...
`PACTRA_NODE_ROOT=`, `PACTRA_NODE_WORKER1=`, `PACTRA_NODE_WORKER2=`,
`PACTRA_NODE_WORKER3=` — one per key, in the order it printed the addresses.
Those lines hold ids and never keys, so listing them is safe; read no other line:

```bash
grep '^PACTRA_NODE_' ~/.pactra/pactra.env
sed -i '' 's/^PACTRA_NODE_ROOT=$/PACTRA_NODE_ROOT=0x<node id>/' ~/.pactra/pactra.env   # Linux: sed -i
grep '^PACTRA_NODE_ROOT=' ~/.pactra/pactra.env    # confirm it took
```
...
`pactra_spawn`, can: the daemon generates the child's key, writes it to
`PACTRA_KEY_FILE` before sending, and fills in the node id itself, so 3b is not
...
> **Check:** `GET https://pactra.example/api/tree/<root>` shows one more node,
...
PACTRA_VAULT=<TreeVault, from the table above>
PACTRA_REGISTRY=<MandateRegistry, from the same table>
PACTRA_RECORD=<ConductRecord — without it, refusals are enforced and never published>
...
| | |
|---|---|
| an MCP client | the config block above, plus `PACTRA_MCP_NODE` if the keyring holds several |
...
curl -s -X POST localhost:8402/fetch \
  ...
  -d '{"node":"0x…","url":"https://attest.pactra.example/attest/894124"}'
```
...
> **Check:** `https://pactra.example/refusal/<id>`. The decision is a
...
"$HOME/.pactra/pactra-2.env"`, and set `PACTRA_KEY_FILE` to it; open the new
...
The same walk with more prose: <https://pactra.example/docs/walkthrough>.
...
| Tool | Takes |
|---|---|
| `pactra_fetch` | `url`, and optionally a method and a body |
| `pactra_status` | nothing — what is left, and which node is the limit |
| `pactra_spawn` | a label — the child's stated purpose, one line, at most 140 characters, written beside its key and on its ERC-8004 identity — and bounds, narrower than this node's |

`pactra_transfer`, `pactra_pay` and `pactra_send` are absent, permanently. The
...
### Attest — `attest.pactra.example`, x402
...
costs nothing, and Pactra does not touch it.
...
Pactra writes the opposite kind of entry. `ConductRecord` is the only address
...
- resolves at `https://pactra.example/refusal/<id>`, which is a Solidity
...
times under Pactra and three times under a plain shared cap, against criteria
fixed before the first run.

| Condition | Briefs | Spent | Taken by the loop | At risk before any work |
|---|---|---|---|---|
| Nothing goes wrong — Pactra | 3 of 3 | $4.68 | $0.00 | **$0.00** |
| Nothing goes wrong — shared cap | 3 of 3 | $4.68 | $0.00 | $20.00 |
| One worker in a loop — Pactra | **3 of 3** | $13.86 | $9.18 | **$0.00** |
...
- **You cannot stop one payment. You can stop the ten-thousandth.** x402 batch
  ...
  contract can sit in the path of a single payment. Pactra bounds capacity, not
  items.
...
- **An agent holding its own wallet is outside all of this.** Pactra bounds what
...
| Surfaces | React 18, Vite, framer-motion, Privy | Privy holds the key, Pactra holds the bound |
...
| **x402**, scheme `exact` | the seller's challenge, and the endpoint Pactra sells |
...
| **Sub-second deterministic finality** | a draw per purchase is affordable; `PACTRA_POLL_MS` is 250 rather than viem's 4,000 |
...
`PACTRA_G4_VARIANTS=3` runs the adversarial search coarse while iterating; the
...
It writes `~/.pactra/pactra.env` at `0600` and prints **addresses only** — the
...
```bash
node --env-file=.env.live --env-file="$HOME/.pactra/pactra.env" \
  packages/daemon/src/main.ts
```
...
```bash
ops/bin/pactra-publish.sh    # rebuild both bundles and publish them
ops/bin/pactra-check.sh      # ask the deployed surfaces from outside
```

`pactra-check.sh` reads the **bundle**, not the page: `index.html` barely
...
The unit Pactra is built for is one owner running a fan-out of their own
