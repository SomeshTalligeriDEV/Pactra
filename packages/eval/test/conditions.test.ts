...
import { pactraBuyer, sharedCapBuyer } from "../src/conditions.ts";
...
test("under Pactra the brief completes and every seller was really paid", async () => {
  const before = await Promise.all(sellers.list.map((s) => balanceOf(s.payTo)));
  const result = await scriptedAgent.run(sellers, pactraBuyer(world));
...
test("nothing leaves the vault before the work does", async () => {
  /* The figure the comparison turns on. Under Pactra the owner's money is
     still the owner's until a purchase has been evaluated; a shared cap has
     already handed over the whole of it. */
  const pactra = pactraBuyer(world);
  assert.equal(pactra.exposureAtStart6, 0n);
...
test("the shared cap completes the same task, which is why it is the control", async () => {
  ...
  try {
    ...
    assert.equal(result.spent6, taskCost6(world.window6));
    /* One transaction against Pactra's two. The fence costs a write per
...
test("a purchase larger than a tranche is refused, and the refusal names the bound", async () => {
  ...
  try {
    const purchase = await pactraBuyer(world).buy("left", greedy.byId(SOURCES[0]!.id).url);
...
test("a worker in a loop is stopped at its own bound, and its sibling still finishes", async () => {
  ...
  try {
    const result = await faultyAgent.run(shop, pactraBuyer(narrow));
