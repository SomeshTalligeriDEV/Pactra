...
export interface EvalRun {
  ...
  scenarios: Scenario[];
  /**
   * `work-gets-through` requires Pactra to deliver the brief in every run of
...
async function scenario(
  ...
  try {
    const pactra = await condition(world, async () => pactraBuyer(world), runs, agent);
...
export async function runEval(opts: { runs?: number; port?: number } = {}): Promise<EvalRun> {
  ...
  const pactraOf = (s: Scenario) => s.conditions.find((c) => c.id === "pactra")!;
  const delivered =
    pactraOf(work).completed === runs &&
    pactraOf(loop).completed === runs &&
    pactraOf(work).refusals === 0 &&
    pactraOf(work).unsupported === 0 &&
    pactraOf(loop).unsupported === 0;
