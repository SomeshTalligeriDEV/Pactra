...
export interface Buyer {
   ...
   *
   * Zero under Pactra and the whole cap under a shared balance, and it is a
 ...
 */
export function pactraBuyer(world: World): Buyer {
  const gates: Partial<Record<WorkerId, Gate>> = {};

  for (const id of ["left", "right"] as WorkerId[]) {
    const worker = world.tree.workers[id];
    const label = id.toUpperCase();
    process.env[`PACTRA_KEY_${label}`] = worker.key;
    gates[id] = new Gate(load({
      PACTRA_RPC: world.rpc,
      PACTRA_CHAIN_ID: "31337",
      PACTRA_VAULT: world.vault,
      PACTRA_REGISTRY: world.registry,
      PACTRA_USDC: world.usdc,
      PACTRA_NETWORKS: "eip155:31337",
      /* anvil mines instantly; the default spends four seconds per draw. */
      PACTRA_POLL_MS: "50",
      PACTRA_ASSETS: world.usdc,
      [`PACTRA_NODE_${label}`]: worker.node,
      [`PACTRA_KEY_${label}`]: worker.key,
  ...
  return {
    ...
    async buy(worker, url) {
      ...
      const result = await pactraFetch(
