...
`pactra-run` and `pactra-proxy` — are not on anybody's PATH yet; the entry
point is the same either way, and `run` is an optional first word, so this
becomes `pactra run …` the day `npm publish` puts it there.
...
| Seller was paid | the seller's response, plus `pactra-paid` and `pactra-transaction` |
| The contract refused | **402**, with `pactra-refusal`, `pactra-breached-at`, `pactra-transaction` and the refusal in the body |
...
payment-required at all handles this without knowing Pactra exists.
...
cannot pay through it. `pactra run` therefore terminates TLS, using a
...
A program launched some other way — no `pactra run`, no CA in its environment —
fails the handshake loudly rather than silently talking to a middlebox.

Running `pactra-proxy` on its own defaults to plaintext and **refuses**
...
```bash
PACTRA_NODE=0x7f3a… PACTRA_DAEMON=http://127.0.0.1:8402 PACTRA_CA=1 \
  node src/main.ts
```

`PACTRA_NODE` and `PACTRA_DAEMON` are read by `run.ts` too, so a container that
sets them needs neither flag.

| Variable | |
|---|---|
| `PACTRA_NODE` | the mandate node this proxy acts for. One proxy, one node |
| `PACTRA_DAEMON` | defaults to `http://127.0.0.1:8402` |
| `PACTRA_PROXY_PORT` | defaults to `8403` |
| `PACTRA_CA` | `1` to terminate TLS. Off by default |
...
The `pactra run` suite drives **curl** — a program that was not written for
Pactra, has no Pactra library in it, and reads `http_proxy` because every HTTP
