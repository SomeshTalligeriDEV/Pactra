#!/usr/bin/env node
/**
 * `pactra run --node 0x… -- python my_agent.py`
 *
 ...
 * genuinely needs an unproxied hop can set `NO_PROXY` before `pactra run`, and
...
function usage(message: string): never {
  ...
  console.error("");
  console.error("usage: pactra run --node <0x…64> [--daemon <url>] -- <command> [args…]");
  process.exit(2);
}

function parse(argv: string[]): Args {
  /* `pactra run …` and `pactra-run …` should both work; the word `run` is a
     subcommand only when the binary was invoked under the shorter name. */
  const args = argv[0] === "run" ? argv.slice(1) : argv;

  let node = process.env.PACTRA_NODE ?? "";
  let daemon = process.env.PACTRA_DAEMON ?? "http://127.0.0.1:8402";
  let i = 0;
  for (; i < args.length; i++) {
    const arg = args[i];
    if (arg === "--") { i++; break; }
