 ...
 * - It is never installed into a system or browser trust store. `pactra run`
 ...
 *
 * A program launched some other way — no `pactra run`, no CA in its
...
export function createEphemeralCa(): CertAuthority {
  const dir = mkdtempSync(join(tmpdir(), "pactra-ca-"));
  ...
  openssl(
    [
      ...
      "-subj", "/CN=Pactra local proxy (ephemeral)",
