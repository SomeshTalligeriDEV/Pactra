 ...
 * its proxy some other way. `pactra run` is the shorter path for anything you
 ...
 *
 * Without `PACTRA_CA=1` this serves plaintext only and refuses CONNECT. That
...
const node = process.env.PACTRA_NODE;
if (!node || !/^0x[0-9a-fA-F]{64}$/.test(node)) {
  console.error("PACTRA_NODE must be the mandate node this proxy acts for");
  process.exit(2);
}

const daemonUrl = process.env.PACTRA_DAEMON ?? "http://127.0.0.1:8402";
const port = Number(process.env.PACTRA_PROXY_PORT ?? 8403);

let ca: CertAuthority | undefined;
if (process.env.PACTRA_CA === "1") {
  try {
...
server.listen(port, () => {
  console.log(`pactra proxy on :${port}`);
  console.log(`  node    ${node}`);
  console.log(`  daemon  ${daemonUrl}`);
  console.log(
    ca
      ? `  tls     terminated; trust ${ca.certPath} in the client, and nowhere else`
      : "  tls     not terminated; CONNECT is refused. Set PACTRA_CA=1 to change that",
  );
  console.log("");
