 ...
 * this without knowing Pactra exists. The refusal itself — which bound, which
...
export function render(result: FetchResult, res: ServerResponse): void {
  if (!result.paid && !result.free) {
    ...
    const headers: Record<string, string | number> = {
      ...
      "pactra-refusal": refusal.reason,
    };
    if (refusal.breachedAt) headers["pactra-breached-at"] = refusal.breachedAt;
    if (refusal.txHash) headers["pactra-transaction"] = refusal.txHash;
  ...
  if (result.paid) {
    headers["pactra-paid"] = String(result.offer.amount);
    if (result.draw.txHash) headers["pactra-transaction"] = result.draw.txHash;
  }
  ...
  const handle = async (origin: string, req: IncomingMessage, res: ServerResponse) => {
    try {
      ...
      if (url.protocol !== "http:" && url.protocol !== "https:") {
        return fail(res, 400, `pactra does not proxy ${url.protocol}`);
      }
    ...
    if (!deps.ca) {
      ...
      socket.end(
        "HTTP/1.1 502 Bad Gateway\r\ncontent-type: text/plain\r\n\r\n" +
          "pactra proxy has no certificate authority, so it cannot read a 402 " +
          "inside TLS. Start it through `pactra run`, which makes one for the " +
