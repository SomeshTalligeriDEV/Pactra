/**
 * `pactra run`, exercised the way the claim is stated: a program nobody
 ...
 * Pactra, it has no Pactra library in it, and it reads `http_proxy` because
...
/** Run a command through `pactra run` and collect what it printed. */
function pactraRun(command: string[]): Promise<{ code: number; stdout: string; stderr: string }> {
...
test("an unmodified program buys through pactra run, having been given no key and no code change", async () => {
  const result = await pactraRun(curl("http://seller.example/apis/v2/scholar"));
...
test("the same program, refused, gets 402 rather than a hang or an empty success", async () => {
  const result = await pactraRun(curl("http://seller.example/expensive"));

  const { body, status } = split(result.stdout);
  assert.equal(status, "402", result.stderr);
  assert.equal(JSON.parse(body).pactra, "refused");
...
test("https works too, because every seller in the catalogue is on https", async () => {
  const result = await pactraRun(curl("https://seller.example/apis/v2/scholar"));
...
test("the same program without pactra run reaches nothing, because the host does not exist", async () => {
...
test("the run's exit code is the program's own", async () => {
  const result = await pactraRun(["node", "-e", "process.exit(3)"]);
