...
`PACTRA_ATTEST_KEY=0x…` in front of a command lands in shell history.

Live at **<https://attest.pactra.example>**, behind nginx on the box that also
serves the site.
...
the daemon reads on the buying side, so a Pactra-bounded agent can pay for an
