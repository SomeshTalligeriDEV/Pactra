 ...
 *
 *     node --env-file="$HOME/pactra/.env.attest" src/main.ts --chain 5042002
 *
...
const chainId = Number(flag(argv, "chain") ?? process.env.PACTRA_CHAIN_ID ?? ARC.chainId);
const rpc = flag(argv, "rpc") ?? process.env.PACTRA_RPC ?? ARC.rpc;
const port = Number(flag(argv, "port") ?? process.env.PACTRA_ATTEST_PORT ?? 8405);
const fromArg = flag(argv, "from") ?? process.env.PACTRA_FROM_BLOCK;
const snapshot = flag(argv, "out") ?? resolve(here, `../ledger.${chainId}.json`);

const key = process.env.PACTRA_ATTEST_KEY as Hex | undefined;
if (!key || !/^0x[0-9a-fA-F]{64}$/.test(key)) {
  console.error("PACTRA_ATTEST_KEY is not set. It is the account that submits");
...
const asset = (process.env.PACTRA_ATTEST_ASSET ?? ARC.erc20) as Address;
...
const payTo = (process.env.PACTRA_ATTEST_PAYTO ?? privateKeyToAccount(key).address) as Address;
...
const bind = flag(argv, "bind") ?? process.env.PACTRA_BIND ?? "127.0.0.1";
