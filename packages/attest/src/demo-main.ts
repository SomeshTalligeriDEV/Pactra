 ...
 *
 *     node --env-file="$HOME/pactra/.env.attest" src/demo-main.ts
 *
...
const chainId = Number(process.env.PACTRA_CHAIN_ID ?? ARC.chainId);
const rpc = process.env.PACTRA_RPC ?? ARC.rpc;
const port = Number(process.env.PACTRA_DEMO_PORT ?? 8406);
const bind = process.env.PACTRA_BIND ?? "127.0.0.1";

const key = process.env.PACTRA_ATTEST_KEY as Hex | undefined;
if (!key || !/^0x[0-9a-fA-F]{64}$/.test(key)) {
  console.error("PACTRA_ATTEST_KEY is not set. The demo seller submits settlements");
...
const payTo = (process.env.PACTRA_DEMO_PAYTO ?? privateKeyToAccount(key).address) as Address;
