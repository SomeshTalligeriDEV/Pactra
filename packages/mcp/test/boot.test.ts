...
const base: Record<string, string> = {
  PACTRA_VAULT: "0x00ab57acd260c594a661b6101bdf7e92267af135",
  PACTRA_REGISTRY: "0x0000000000000000000000000000000000000001",
  PACTRA_NODE_ONE: ONE,
  PACTRA_KEY_ONE: KEY_ONE,
  PACTRA_NODE_TWO: TWO,
  PACTRA_KEY_TWO: KEY_TWO,
  PACTRA_RPC: "http://127.0.0.1:1",
};
...
function firstWord(child: ChildProcess): Promise<string> {
  return new Promise<string>((resolve, reject) => {
    ...
    child.stderr!.on("data", (chunk) => {
      text += String(chunk);
      if (/pactra mcp: node|holds no key|Error/.test(text)) {
        clearTimeout(gaveUp);
        resolve(text);
      }
    });
...
test("the entry point starts and announces its node", async () => {
  ...
  child.kill();
  assert.match(said, /pactra mcp: node 0x1111/, said);
});

test("the node it speaks for can be named rather than ordered into", async () => {
  const child = start({ PACTRA_MCP_NODE: TWO });
  const said = await firstWord(child);
  child.kill();
  assert.match(said, /pactra mcp: node 0x2222/, said);
});

test("a node this process holds no key for is refused by name", async () => {
  const child = start({ PACTRA_MCP_NODE: `0x${"99".repeat(32)}` });
  const said = await firstWord(child);
  child.kill();
  assert.match(said, /holds no key for it/, said);
  assert.doesNotMatch(said, /pactra mcp: node 0x/, said);
});
