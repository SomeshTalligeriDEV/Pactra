...
/**
 * The gate for this step: Pactra runs as an MCP server and refuses on cue.
 *
...
before(async () => {
  ...
  process.env.PACTRA_KEY_CHILD = OP_CHILD_KEY;

  const gate = new Gate(load({
    PACTRA_RPC: h.rpc, PACTRA_CHAIN_ID: "31337",
    PACTRA_VAULT: h.vault, PACTRA_REGISTRY: h.registry, PACTRA_USDC: h.usdc,
    PACTRA_NETWORKS: "eip155:31337", PACTRA_ASSETS: h.usdc,
    PACTRA_NODE_CHILD: h.child, PACTRA_KEY_CHILD: OP_CHILD_KEY,
...
test("an agent asks for a URL and gets a paid body, having named no recipient", async () => {
  ...
  const shown = say(await client.callTool({ name: "pactra_fetch", arguments: { url: h.sellerUrl } }));
...
test("it refuses on cue, in words, with the record", async () => {
  const shown = say(await client.callTool({ name: "pactra_fetch", arguments: { url: h.greedyUrl } }));
...
test("status reads the chain, not a cache", async () => {
  const shown = say(await client.callTool({ name: "pactra_status", arguments: {} }));
