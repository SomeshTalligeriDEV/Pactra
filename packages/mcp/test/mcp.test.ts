...
test("pactra_status says when an ancestor is the limit, not this node", async () => {
  const client = await connect(stubGate());
  const result = await client.callTool({ name: "pactra_status", arguments: {} });
...
test("pactra_spawn never hands the agent a key", async () => {
  const client = await connect(stubGate());
  const result = await client.callTool({
    name: "pactra_spawn",
...
test("pactra_spawn states what the child is for, and says it is not a bound", async () => {
