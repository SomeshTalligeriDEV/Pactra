#!/usr/bin/env node
/**
 * Pactra inside an MCP client. One config block, and the agent in front of you
 * is bounded by a contract it cannot reach.
 *
 *   { "mcpServers": { "pactra": {
 *       "command": "npx", "args": ["-y", "pactra-mcp"],
 *       "env": { "PACTRA_ENV_FILE": "/Users/<you>/.pactra/pactra.env",
 *                "PACTRA_MCP_NODE": "0x…" } } } }
 *
 * From a clone, `node /path/to/pactra/packages/mcp/src/main.ts` with the same
...
const envFiles = (process.env.PACTRA_ENV_FILE ?? "")
  .split(",")
  .map((file) => file.trim())
  .filter(Boolean);
for (const file of envFiles) {
  try {
    ...
    console.error(`pactra: PACTRA_ENV_FILE names ${file}, which could not be read: ${(error as Error).message}`);
    process.exit(1);
  }
}
/* A spawned child's key is written to the file the keys came from, not to a
   default that may be a different tree's. */
if (envFiles.length > 0) process.env.PACTRA_KEY_FILE ??= envFiles[envFiles.length - 1];

/* The deployment this build was made against, so a config block needs no
   addresses. An explicit variable still wins. */
if (DEPLOYMENT) {
  process.env.PACTRA_VAULT ??= DEPLOYMENT.vault;
  process.env.PACTRA_REGISTRY ??= DEPLOYMENT.registry;
  process.env.PACTRA_RECORD ??= DEPLOYMENT.record;
}
...
const asked = process.env.PACTRA_MCP_NODE?.toLowerCase();
const node = asked ? configured.find((n) => n.toLowerCase() === asked) : configured[0];
if (!node) {
  console.error(
    asked
      ? `pactra: PACTRA_MCP_NODE is ${asked} and this process holds no key for it. It holds: ${configured.join(", ") || "none"}`
      : "pactra: no node configured; set PACTRA_NODE_<label> and PACTRA_KEY_<label>",
  );
  process.exit(1);
}
...
console.error(`pactra mcp: node ${node} on chain ${config.chainId}`);
