...
Pactra inside an MCP client. One config block, and the agent in front of you is
...
`PACTRA_ENV_FILE` is the key file `pactra-init` wrote, as an absolute path —
...
version was built against; `PACTRA_VAULT`, `PACTRA_REGISTRY` and
`PACTRA_RECORD` override them. Several files can be listed, comma-separated.

Node 22 or newer. The operator keys, the mandate and the daemon setup are in
the [repository](https://github.com/SomeshTalligeriDEV/pactra) and at
<https://pactra.example/docs/walkthrough>.

Where the keyring holds several nodes, `PACTRA_MCP_NODE` names the one this
...
| | |
|---|---|
| `pactra_fetch(url, method?, body?)` | fetch, and pay if the seller asks. The recipient and the price come from the seller's own challenge |
| `pactra_status()` | what this mandate may still draw, and which node is the limit — often an ancestor |
| `pactra_spawn(label, budgetUsdc, …)` | a child mandate, narrower than this one. The key stays here |

## The tools that are missing, permanently

`pactra_transfer`, `pactra_pay`, `pactra_send`.
