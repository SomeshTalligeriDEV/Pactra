...
server.listen(config.port, () => {
  console.log(`pactra daemon on :${config.port}`);
  console.log(`  chain    ${config.chainId} via ${config.rpcUrl}`);
  console.log(`  vault    ${config.vault}`);
  console.log(`  nodes    ${gate.nodes().join(", ")}`);
  /* Named because its default is `$HOME/.pactra/pactra.env` whatever
  ...
  console.log(
    gate.recorder.enabled
      ...
      : "  record   none. Refusals are enforced and never published; set PACTRA_RECORD",
  );
});
