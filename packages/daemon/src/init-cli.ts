/**
 * `pactra-init` — the command around `initOperators`.
 *
...
const CONSOLE_URL = process.env.PACTRA_CONSOLE_URL ?? "https://pactra.example/console";
...
const path = resolve(flag("out") ?? `${homedir()}/.pactra/pactra.env`);
...
try {
  ...
  console.log(`node id in PACTRA_NODE_<label> in that file.`);
  console.log("");
  /* The addresses, not a sentence pointing at where the addresses live. This
     ...
     `PACTRA_VAULT is not set` with nothing on screen saying where to find one. */
  if (DEPLOYMENT) {
    ...
    console.log(`  PACTRA_VAULT=${DEPLOYMENT.vault}`);
    console.log(`  PACTRA_REGISTRY=${DEPLOYMENT.registry}`);
    console.log(`  PACTRA_RECORD=${DEPLOYMENT.record}`);
    console.log(`  PACTRA_RPC=${ARC.rpc}`);
    console.log(`  PACTRA_PORT=8402`);
    ...
    console.log(`  PACTRA_KEY_FILE=${path}`);
    console.log("  ENV");
  } else {
    console.log("This build carries no deployment, so the contract addresses for");
    console.log("PACTRA_VAULT, PACTRA_REGISTRY and PACTRA_RECORD have to come from");
