 ...
 * a newline in it would start a line of its own — `PACTRA_KEY_ROOT=…` smuggled
...
export function keyFileAt(path: string): KeyFile {
  return {
    ...
    remember(secret, operator, purpose) {
      ...
      if (new RegExp(`^PACTRA_KEY_${label}=`, "m").test(existing)) {
    ...
    purposes() {
      ...
      for (const line of lines) {
        ...
        const node = /^PACTRA_NODE_[A-Za-z0-9_]+=(0x[0-9a-fA-F]{64})$/.exec(line);
        if (node) {
        ...
        if (!/^PACTRA_KEY_/.test(line) && line.trim() !== "" && !line.startsWith("#")) said = undefined;
      }
      return found;
    },
    ...
    bind(label, node) {
      ...
      const empty = new RegExp(`^PACTRA_NODE_${label}=$`, "m");
      if (!empty.test(current)) {
        throw new Error(`${path} has no empty PACTRA_NODE_${label} to fill`);
      }
      ...
      const next = current.replace(empty, `PACTRA_NODE_${label}=${node}`);
      const staging = `${path}.tmp-${process.pid}`;
