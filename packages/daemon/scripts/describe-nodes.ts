 ...
 *
 * Nothing here can change a bound. `pactra.purpose` is metadata on an identity;
...
if (!config.record) {
  console.error("PACTRA_RECORD is not set, so no node has an identity to write to.");
  process.exit(2);
}
...
for (const [node, purpose] of Object.entries(PURPOSE)) {
  ...
  /* `cleanPurpose` and not a length check of this script's own. It is the one
     ...
     the key file, and `PACTRA_KEY_ROOT=…` smuggled in that way is a key the
