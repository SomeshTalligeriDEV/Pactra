 ...
 *
 *     PACTRA_ROOT=0x… node scripts/record-g2.mjs
 *
...
const chainId = Number(process.env.PACTRA_CHAIN_ID ?? 5042002);
const deployment = JSON.parse(
  readFileSync(resolve(here, `../../contracts/deployments/${chainId}.json`), "utf8"),
);
const rpc = process.env.PACTRA_RPC ?? "https://rpc.blockdaemon.testnet.arc.io";
const meter = process.env.PACTRA_METER ?? "https://pactra.example/api";
const root = process.env.PACTRA_ROOT;
if (!root) {
  console.error("PACTRA_ROOT is not set: G2 is about one tree, and it has to be named");
  process.exit(2);
}
