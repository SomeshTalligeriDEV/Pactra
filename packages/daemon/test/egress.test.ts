 ...
 *
 * The tool list is the argument: there is `pactra_fetch` and there is no
 * `pactra_transfer(to, amount)`, so an agent cannot express "send money to X".
 * That claim is about money and it holds. It was doing duty for a second claim
 * nobody had made good on — `pactra_fetch` takes a method and a body, so it is
