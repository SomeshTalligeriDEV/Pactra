...
const out = () => join(mkdtempSync(join(tmpdir(), "pactra-init-")), "pactra.env");
...
test("no private key reaches the terminal", () => {
  ...
  const keys = [...written.matchAll(/^PACTRA_KEY_[A-Z0-9]+=(0x[0-9a-f]{64})$/gm)].map(
    (m) => m[1]! as Hex,
  );
...
test("the addresses shown are the addresses of the keys written", () => {
  ...
  assert.match(readFileSync(path, "utf8"), new RegExp(`PACTRA_KEY_ROOT=${KNOWN}`));
});
