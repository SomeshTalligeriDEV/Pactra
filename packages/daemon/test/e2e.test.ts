...
test("an agent with no key pays a seller through the daemon", async () => {
  ...
  const result = await pactraFetch({ node: h.child, url: h.sellerUrl }, deps());
...
test("a seller asking past the tranche cap is refused, and nothing is paid", async () => {
  const before = await sellerBalance();
  const result = await pactraFetch({ node: h.child, url: h.greedyUrl }, deps());
...
test("the reason is the one the chain gave, not the one the simulation predicted", async () => {
  ...
  process.env.PACTRA_KEY_DOOMED = OP_CHILD_KEY;
  const gate = new Gate(load({
    PACTRA_RPC: h.rpc, PACTRA_CHAIN_ID: "31337",
    PACTRA_VAULT: h.vault, PACTRA_REGISTRY: h.registry, PACTRA_USDC: h.usdc,
    PACTRA_NETWORKS: "eip155:31337", PACTRA_ASSETS: h.usdc,
    PACTRA_NODE_DOOMED: doomed, PACTRA_KEY_DOOMED: OP_CHILD_KEY,
  } as NodeJS.ProcessEnv));
