...
const freshPath = () => join(mkdtempSync(join(tmpdir(), "pactra-keyfile-")), "pactra.env");
...
test("a spawned key is written at 0600, with its node id left empty", () => {
  ...
  assert.match(text, new RegExp(`^PACTRA_KEY_${LABEL}=${SECRET}$`, "m"));
  assert.match(text, new RegExp(`^PACTRA_NODE_${LABEL}=$`, "m"));
...
test("binding fills that label's node id and leaves every other line alone", () => {
  const path = freshPath();
  const before = "# written by init\nPACTRA_KEY_ROOT=0x11\nPACTRA_NODE_ROOT=0x22";
  ...
  assert.match(text, new RegExp(`^PACTRA_NODE_${LABEL}=${NODE}$`, "m"));
  assert.match(text, new RegExp(`^PACTRA_KEY_${LABEL}=${SECRET}$`, "m"));
...
test("binding a label that was never remembered is refused, not skipped", () => {
  const path = freshPath();
  writeFileSync(path, "PACTRA_NODE_ROOT=\n");
  assert.throws(() => keyFileAt(path).bind("CHILD_DEADBEEF", NODE), /no empty PACTRA_NODE_CHILD_DEADBEEF/);
});
...
test("the daemon writes the child's key before it asks the registry, so a failed spawn loses nothing", async () => {
  ...
  assert.match(readFileSync(path, "utf8"), new RegExp(`^PACTRA_NODE_${LABEL}=$`, "m"));
});
...
test("a spawn that lands leaves the node id beside its key, and never returns the key", async () => {
  ...
  assert.ok(!text.includes(SECRET.slice(2)), "the key is not in the response");
  assert.match(readFileSync(path, "utf8"), new RegExp(`^PACTRA_NODE_${LABEL}=${NODE}$`, "m"));
});
...
test("a spawn the registry refused before broadcasting takes its key back", async () => {
  ...
  assert.doesNotMatch(text2, new RegExp(`PACTRA_KEY_${LABEL}=`), "the key is gone");
  assert.doesNotMatch(text2, new RegExp(`PACTRA_NODE_${LABEL}=`), "and so is its empty node id");
});
...
test("a failure after the transaction was sent keeps the key, because it may have landed", async () => {
  ...
  assert.match(readFileSync(path, "utf8"), new RegExp(`^PACTRA_KEY_${LABEL}=${SECRET}$`, "m"));
});
