...
function gateWith(record?: Address): Gate {
  ...
  return new Gate(
    load({
      ...
      PACTRA_VAULT: h.vault,
      PACTRA_REGISTRY: h.registry,
      PACTRA_USDC: h.usdc,
      PACTRA_IDENTITY: h.identity,
      ...(record ? { PACTRA_RECORD: record } : {}),
      PACTRA_NODE_CHILD: h.child,
      PACTRA_KEY_CHILD: OP_CHILD_KEY,
...
test("a node's identity is its operator's, registered with the operator's own key", async () => {
  ...
  /* Binding checks on chain that the identity's holder is the node's operator.
     Pactra does not hold the token: a record about a token we control is the
