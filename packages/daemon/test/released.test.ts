 ...
 * the order `pactraFetch` does things in and what `ChainReleases` will match.
 */
...
import { pactraFetch, type Transport } from "../src/fetch.ts";
...
test("a released refusal pays that purchase with no draw", async () => {
  ...
  const result = await pactraFetch({ node: NODE, url: "https://seller/arc/snapshot" }, w.deps);
...
test("a release pays one purchase, not one per ask", async () => {
  const w = world([row()]);
  await pactraFetch({ node: NODE, url: "https://seller/arc/snapshot" }, w.deps);
  const again = await pactraFetch({ node: NODE, url: "https://seller/arc/snapshot" }, w.deps);
...
test("what was spent survives a restart", async () => {
  ...
  const again = await pactraFetch({ node: NODE, url: "https://seller/arc/snapshot" }, restarted);
...
test("a release on a cut branch buys nothing, even when the cut came after the release", async () => {
  ...
  const restored = await pactraFetch({ node: NODE, url: "https://seller/arc/snapshot" }, w.deps);
...
test("a settlement that fails leaves the release spendable", async () => {
  ...
  await assert.rejects(pactraFetch({ node: NODE, url: "https://seller/arc/snapshot" }, w.deps), /gateway refused/);
  fail = false;
  const retry = await pactraFetch({ node: NODE, url: "https://seller/arc/snapshot" }, w.deps);
