...
```bash
node --env-file=.env.live --env-file="$HOME/.pactra/pactra.env" src/main.ts
```
...
Writes `~/.pactra/pactra.env` at `0600` and prints **addresses only** — the
...
| `record.ts` | publishing a refusal, which is skipped entirely when `PACTRA_RECORD` is unset — and said out loud at startup |

## Configuration

`PACTRA_VAULT`, `PACTRA_REGISTRY`, `PACTRA_NODE_<label>` and
`PACTRA_KEY_<label>` are required. `PACTRA_RECORD`, `PACTRA_IDENTITY`,
`PACTRA_RPC`, `PACTRA_CHAIN_ID`, `PACTRA_USDC`, `PACTRA_NETWORKS`,
`PACTRA_ASSETS`, `PACTRA_PORT` and `PACTRA_POLL_MS` are optional, and every one
of them is documented at <https://pactra.example/docs/configuration> — from this
