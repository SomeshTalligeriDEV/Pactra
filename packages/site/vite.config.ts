...
export default defineConfig(({ mode }) => {
  ...
  const consoleOrigin = process.env.PACTRA_CONSOLE_ORIGIN ?? env.PACTRA_CONSOLE_ORIGIN ?? "http://127.0.0.1:5173";
  return {
  plugins: [staticLanding(process.env.VITE_CONSOLE_URL ?? env.VITE_CONSOLE_URL), react()],
  build: { rollupOptions: { input: local("./app.html") } },
  resolve: {
    alias: [
  ...
  server: { port: Number(process.env.PORT ?? 5274), host: true, proxy: { "/console": consoleOrigin } },
  preview: { port: Number(process.env.PORT ?? 5280), host: true, proxy: { "/console": consoleOrigin } },
};
});
