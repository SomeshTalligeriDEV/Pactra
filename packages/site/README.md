...
- **Every figure comes from `@pactra/fixtures`.** Nothing here hardcodes a cap,
...
The design system is `@pactra/ui`, consumed as source through a Vite alias. The
...
the way, and the root refuses. Run it with `pactraed={false}` for the
...
is what makes those ids resolve; `ops/bin/pactra-publish.sh` sets it to `/api`
