...
import { PactraProvider } from "pactra-ui";
...
/** Crossing back to the standalone home page must load its HTML document. */
function StaticHome() {
  useEffect(() => {
    const destination = window.location.hash ? `/product${window.location.hash}` : "/";
      ...
      <Routes>
        ...
        {SECTIONS.map((section) => (
          <Route
            key={section.id}
            path={`/${section.id}`}
            element={<Navigate to={{ pathname: "/product", hash: `#${section.id}` }} replace />}
          />
        ))}
        ...
        {/* Not a page anybody navigates to. `ConductRecord.RECORD_BASE` is a
            ...
            every record Pactra writes a record whose linkage goes nowhere. */}
