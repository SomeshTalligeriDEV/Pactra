...
import { useMeasure, usePactraReducedMotion } from "pactra-ui";
import { MANDATE } from "@pactra/fixtures";
...
/* ==========================================================================
   ...
   `pactraed={false}` runs the same tree without ancestor debit: every node
...
/** Where an unpactraed run has made its point and starts over. */
...
export function DelegationTree({
  ...
  const reduced = usePactraReducedMotion();
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => {
      ...
      setSpent((previous) => {
        ...
        /* Pactraed: the root's window is the bound, so the draw is refused
           when this tranche would carry it past 1. Unpactraed: nothing looks
           at the root at all, and every local check still passes. */
        const wouldBreach = rootSpent + TRANCHE_SHARE > 1;
        const refused = pactraed && wouldBreach;
  ...
  }, [pactraed, interval, reduced, onRefuse]);
  ...
  const overRun = pactraed ? 0 : Math.max(0, rootSpent - 1);
  const breaching = overRun > 0.01;

  /* Both endings are terminal. A refused tree has nothing left to draw, and an
     unpactraed one has pinned every ring past its bound and will draw the same
  ...
  const finished = Boolean(draw?.refused) || (!pactraed && rootSpent >= OVERRUN_STOP);
