...
import { Button, Logo } from "pactra-ui";
  ...
  return (
    <header className={`nav${isLifted ? " nav--lifted" : ""}`}>
      <div className={`nav__inner${width === "default" ? "" : ` nav__inner--${width}`}`}>
        ...
        {home.startsWith("#") ? (
          <a href={home} className="nav__brand" aria-label="Pactra">
            {brand}
          </a>
        ) : (
          <Link to={home} className="nav__brand" aria-label="Pactra">
            {brand}
          </Link>
        )}
