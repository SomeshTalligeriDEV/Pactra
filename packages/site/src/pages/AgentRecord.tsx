...
import type { Strength } from "pactra-ui";
import {
  ARC,
  ATTEST,
  ENFORCED_BY,
...
import { shortId, isAddress } from "@pactra/fixtures";
...
function LiveAgentRecord({ data }: { data: LiveConduct }) {
  ...
  usePageMeta({
    title: `Agent ${data.agentId} · conduct record · Pactra`,
...
function PreviewAgentRecord({ node }: { node: ReturnType<typeof flatten>[number] }) {
  usePageMeta({
    title: `Agent ${node.agentId} · conduct record · Pactra`,
