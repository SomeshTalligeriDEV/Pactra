...
import { MCP_CONFIG } from "@pactra/fixtures";
...
export function Introduction() {
  return (
    <>
      <Lead>
        Pactra gives a whole tree of agents one budget and enforces it in a
      ...
      <H2 id="what-pactra-does">What Pactra does</H2>
      <UL>
        <li>
      ...
      <P>
        Pactra cannot block one individual payment. Payments here are signed off
        chain and settled in batches, so there is no contract in the path of a
        single one. What Pactra bounds is capacity.
      </P>
      ...
      <P>
        ...
        and funds it itself, Pactra cannot get in front of the money. In that
        case Pactra can observe and cannot control, and it says so rather than
        implying otherwise.
      </P>
...
export function Quickstart() {
  return (
    <>
      ...
      <P>
        Add Pactra to <C>claude_desktop_config.json</C>. The block points the
      ...
      <Note tone="warn" title="The key stays in its file, not in the agent">
        The server process reads the key from <C>pactra.env</C>; it never enters
      ...
      <P>
        ...
        a paid endpoint returns a payment challenge, and Pactra settles it if
        the contract allows.
      </P>
      <Code>{`Fetch https://api.aisa.one/apis/v2/coingecko/simple/price?ids=bitcoin
through pactra_fetch and tell me the price.`}</Code>
  ...
  "transaction": "0x9c21…"
      ...
      <P>
        See <Link to="/docs/proxy">Proxy and pactra run</Link> for how it handles
  ...
  return (
    <>
      <Lead>
        ...
        one is off chain, costs nothing, and Pactra does not touch it.
      </Lead>
