...
import { ARC, ATTEST, DEPLOYMENT, MCP_CONFIG, REASONS, REASON_MEANING, formatUsdc } from "@pactra/fixtures";
...
export function Integrate() {
  ...
  return (
    <>
      <Lead>
        Pactra goes between your agent and the money, not inside your agent.
      ...
      <Defs
        items={[
          ...
          },
          {
            ...
            def: "pactra run in front of it. It sets the proxy variables the runtime already reads.",
          },
          {
            term: "Anything else",
      ...
      <P>
        ...
        as <C>pactra-mcp</C>, so there is nothing to clone for this part. It
        reads the key from the file <C>init</C> wrote, named in{" "}
        <C>PACTRA_ENV_FILE</C>, so every key keeps one copy — in the file{" "}
      ...
      <P>
        A keyring holding more than one node also takes <C>PACTRA_MCP_NODE</C>,
      ...
      <P>
        ...
        That belongs in the agent's own instructions, and Pactra ships a file
...
pactra_fetch may come back refused. That is the contract declining, not an
...
pactra_status says what is left and which node in the tree is the limit. It is
often an ancestor, so a balance above this agent is not this agent's to spend.`}</Code>
      ...
      <P>
        ...
        changes: <C>pactra run</C> sets the proxy variables its HTTP client
      ...
      <P>
        See <Link to="/docs/proxy">Proxy and pactra run</Link> for what it does
...
curl -s -X POST localhost:8402/fetch \\
  ...
  "offer": { "amount": "10000", "payTo": "0x3feeA285…", "asset": "0x3600…0000" }
      ...
      <P>
        Pactra bounds what passes the gate. It cannot bound money that never
        came through the gate, and an agent holding its own wallet is spending
        money Pactra can only watch. The docs say this in{" "}
      ...
      <P>
        Every refusal Pactra writes is published to a registry that is not ours,
      ...
      <Code lang="bash">{`curl -s https://attest.pactra.example${ATTEST.resourcePath}/894124`}</Code>
      <P>
...
export function Walkthrough() {
  return (
    <>
      ...
      <Code lang="bash">{`git clone https://github.com/SomeshTalligeriDEV/pactra.git && cd pactra
npm ci --prefix packages/daemon
npm run init --prefix packages/daemon -- --nodes 4`}</Code>
      <P>
        This writes <C>~/.pactra/pactra.env</C> at <C>0600</C> and prints{" "}
      ...
      <Note tone="good" title="Check">
        <C>GET https://pactra.example/api/tree/&lt;root&gt;</C> returns your root.
      </Note>
      ...
      <P>
        <C>init --nodes 4</C> wrote four empty lines — <C>PACTRA_NODE_ROOT=</C>,{" "}
        <C>PACTRA_NODE_WORKER1=</C>, <C>PACTRA_NODE_WORKER2=</C>,{" "}
        <C>PACTRA_NODE_WORKER3=</C> — one per key, in the order it printed the
      ...
      <Code lang="bash">{`grep '^PACTRA_NODE_' ~/.pactra/pactra.env

# macOS; on Linux drop the '' after -i
sed -i '' 's/^PACTRA_NODE_ROOT=$/PACTRA_NODE_ROOT=0x<node id>/' ~/.pactra/pactra.env

grep '^PACTRA_NODE_ROOT=' ~/.pactra/pactra.env   # confirm it took`}</Code>
      <P>
      ...
      <Note tone="info" title="A stated purpose needs the operator">
        ...
        <C>purpose</C>, or <C>pactra_spawn</C>, can: one line, at most 140
        characters, written beside the key and on the child's ERC-8004
        identity. The daemon writes that key to <C>PACTRA_KEY_FILE</C> before
...
for n in json.load(sys.stdin)['nodes']:
    print(n['node'][:12], 'operator', n['mandate']['operator'])"`}</Code>
      ...
      <P>
        ...
        three. Where the key file holds several nodes, <C>PACTRA_MCP_NODE</C>{" "}
        names which one an MCP server speaks for.
      </P>

      <H2 id="seven">7. Buy something</H2>
      <P>Pactra sells a reading for a cent, so there is always something priced:</P>
      <Code lang="bash">{`curl -s -X POST localhost:8402/fetch \\
  -H 'content-type: application/json' \\
  -d '{"node":"0x…","url":"https://attest.pactra.example/attest/894124"}'`}</Code>
      <OL>
      ...
      <Note tone="good" title="Check">
        <C>https://pactra.example/refusal/&lt;id&gt;</C> shows the decision and
      ...
      <OL>
        ...
        <li>
          ...
          <C>PACTRA_KEY_FILE</C> at it:{" "}
          <C>npm run init --prefix packages/daemon -- --nodes 4 --out "$HOME/.pactra/pactra-2.env"</C>
        </li>
        <li>
...
export function DocsIndex() {
  return (
    <>
      <Lead>
        Pactra gives a tree of agents one budget and enforces it in a contract.
