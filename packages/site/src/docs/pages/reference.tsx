...
import { ARC, DRILL, ERC8004, EVAL, GATES, SEARCH, formatUsdc } from "@pactra/fixtures";
...
Reputation  ${ERC8004.reputation}`}</Code>
      ...
      <Code lang="bash">{`cast wallet import pactra-deployer --interactive   # once
  ...
  return (
    <>
      ...
      <Table
        ...
        rows={[
          [<C key="v">PACTRA_VAULT</C>, "TreeVault address, from the deployment file"],
          [<C key="r">PACTRA_REGISTRY</C>, "MandateRegistry address, from the same file"],
          [<C key="n">PACTRA_NODE_&lt;label&gt;</C>, "the mandate node this daemon acts for"],
          [<C key="k">PACTRA_KEY_&lt;label&gt;</C>, "the operator key for that node. The agent never sees it"],
        ]}
      />
      ...
      <Table
        ...
        rows={[
          [<C key="rec">PACTRA_RECORD</C>, "unset. Without it, refusals are enforced and never published"],
          [<C key="id">PACTRA_IDENTITY</C>, "the live ERC-8004 Identity registry"],
          [<C key="rpc">PACTRA_RPC</C>, ARC.rpc],
          [<C key="cid">PACTRA_CHAIN_ID</C>, `${ARC.chainId}`],
          [<C key="usdc">PACTRA_USDC</C>, "the 6 decimal view of USDC on Arc"],
          [<C key="net">PACTRA_NETWORKS</C>, "the networks this daemon will settle on"],
          [<C key="as">PACTRA_ASSETS</C>, "the assets it will pay in"],
          [<C key="port">PACTRA_PORT</C>, "8402"],
          [<C key="poll">PACTRA_POLL_MS</C>, "250, matched to Arc's finality rather than to viem's 4,000"],
          [
            <C key="mcpnode">PACTRA_MCP_NODE</C>,
            "unset, and then the MCP server speaks for whichever key parsed first. Read only by the MCP server, which serves one node",
          ],
          [
            <C key="keyfile">PACTRA_KEY_FILE</C>,
            "~/.pactra/pactra.env. Where a key minted at run time by a spawn is written — before the spawn is sent, so a restart cannot lose it",
          ],
        ]}
      />
      ...
      <Table
        head={["Variable", "Meaning"]}
        rows={[
          [<C key="pn">PACTRA_NODE</C>, "the node this proxy acts for. One proxy, one node"],
          [<C key="pd">PACTRA_DAEMON</C>, "where the daemon is. Defaults to localhost:8402"],
          [<C key="pp">PACTRA_PROXY_PORT</C>, "8403"],
          [<C key="pc">PACTRA_CA</C>, "set to 1 to terminate TLS. Off by default"],
        ]}
      />
      ...
      <P>
        ...
        <C>PACTRA_METER_MAX_BLOCKS</C>, <C>PACTRA_METER_CHUNK</C>,{" "}
        <C>PACTRA_METER_PACE_MS</C> and <C>PACTRA_METER_INTERVAL_MS</C>, so a
      ...
      <Table
        head={["Variable", "Meaning"]}
        rows={[
          [<C key="ak">PACTRA_ATTEST_KEY</C>, "the account that submits settlements and pays their gas. Required"],
          [<C key="ap">PACTRA_ATTEST_PAYTO</C>, "who is paid. Defaults to the submitter's own address"],
          [<C key="aa">PACTRA_ATTEST_ASSET</C>, "the token payments arrive in. Defaults to the 6 decimal view of USDC on Arc"],
          [<C key="ao">PACTRA_ATTEST_PORT</C>, "8405"],
        ]}
      />
...
function scenarioRows(index: number) {
  const [pactra, shared] = EVAL.scenarios[index]!.conditions;
  const both = [pactra!, shared!];
  return [
...
closest any strategy came: ${formatUsdc(SEARCH.closestToBudget6)} of ${formatUsdc(SEARCH.budget6, 0)}`}</Code>
      ...
      <P>
        Every other gate here measures whether Pactra refuses. G7 measures the
      ...
      <P>
        ...
        root and its two workers — run {EVAL.runs} times under Pactra and{" "}
      ...
      <Table
        head={["", "Pactra", "A plain shared cap"]}
        rows={scenarioRows(0)}
      />
      <P>
      ...
      <P>
        ...
        Pactra the loop was refused by a bound on its own node, and the
  ...
  return (
    <>
      ...
      <P>
        ...
        agent's hands, which Pactra deliberately does not: the agent holds no
        key at all.
      </P>
      ...
      <P>
        ...
        prevented at the gate. Pactra states this rather than implying the
      ...
      <H2 id="what-does-it-cost">What does Pactra cost to run?</H2>
      <P>
        There is no fee to Pactra. The costs are gas on Arc for a draw or a
      ...
      <P>
        ...
        off chain signature and Pactra adds nothing to it — the fast path stays
      ...
      <H2 id="what-happens-if-pactra-goes-away">What happens if Pactra goes away?</H2>
      <P>
