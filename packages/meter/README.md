...
Live at **<https://pactra.example/api/>** — the process binds `127.0.0.1:8404`
...
| Variable | Default | |
|---|---|---|
| `PACTRA_METER_PORT` | `8404` | loopback only; nginx is the door |
| `PACTRA_METER_CHUNK` | `1000` | blocks per `eth_getLogs` |
| `PACTRA_METER_MAX_BLOCKS` | `20000` | blocks per pass, so a backfill resumes rather than stalls |
| `PACTRA_METER_PACE_MS` | `150` | between chunks |
| `PACTRA_METER_INTERVAL_MS` | `5000` | between passes, once caught up |
