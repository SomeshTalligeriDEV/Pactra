...
function parse(argv: string[]): Args {
  ...
  const chainId = Number(args.chain ?? process.env.PACTRA_CHAIN_ID ?? ARC.chainId);
  return {
    chainId,
    rpc: args.rpc ?? process.env.PACTRA_RPC ?? ARC.rpc,
    ...
    fromBlock: args.from ?? process.env.PACTRA_FROM_BLOCK,
    out: args.out ?? resolve(here, `../ledger.${chainId}.json`),
    port: Number(args.port ?? process.env.PACTRA_METER_PORT ?? 8404),
    bind: args.bind ?? process.env.PACTRA_BIND ?? "127.0.0.1",
    once: flags.has("once"),
    intervalMs: Number(args.interval ?? process.env.PACTRA_METER_INTERVAL_MS ?? 5_000),
    maxBlocks: BigInt(args["max-blocks"] ?? process.env.PACTRA_METER_MAX_BLOCKS ?? 20_000),
    chunk: BigInt(args.chunk ?? process.env.PACTRA_METER_CHUNK ?? 1_000),
    paceMs: Number(args.pace ?? process.env.PACTRA_METER_PACE_MS ?? 150),
  };
}
