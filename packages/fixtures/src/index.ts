/**
 * Pactra fixtures — the only source of any figure.
 *
...
export const GATEWAY = {
   ...
   * one off-chain signature and no on-chain step for Pactra to sit in. Any
...
/* ------------------------------------------------------------------ */
/* The attestation endpoint — what Pactra itself sells                 */
/* ------------------------------------------------------------------ */
 ...
 * 13 September, every one on a mainnet — so a Pactra mandate on Arc testnet
 ...
 * something that is not Pactra's own record.
 *
...
export const DEMO_SELLER = {
  ...
  host: "demo-seller.pactra.example",
  minLeadSeconds: 15,
  maxTimeoutSeconds: 300,
} as const;
 ...
 *
 * Most of what Pactra shows is `enforced`: the money does not move unless the
 ...
 * bug in Pactra rather than as a bound it hit.
 */
 ...
 *
 * Every other gate measures whether Pactra refuses. This one measures whether
 ...
 * `pactra_spawn(label, cap)` — an argument list the server has never had.
 */
