 ...
 *
 * - **Linkage.** Every record Pactra writes resolves to a refusal in the
...
contract G6_Record is Base {
    ...
    function setUp() public override {
        ...
        /* Each daemon registers its own identity with its own key. Pactra does
           not register it and does not hold it — a record about a token we
           control is the shape of the thing being replaced. */
        vm.prank(opRoot);
        agentRoot = identity.register("https://pactra.example/agent/orchestrator");
        vm.prank(opA);
        agentA = identity.register("https://pactra.example/agent/research-worker");
    ...
    function test_the_record_carries_the_amount_the_contract_refused_and_not_a_number_anyone_chose() public {
        ...
        assertEq(f.tag1, "pactra.refused", "one tag, one kind of record");
    ...
    function test_the_record_is_filed_against_the_agent_that_drew_not_the_ancestor_that_refused() public {
        ...
        vm.prank(opG);
        uint256 agentG = identity.register("https://pactra.example/agent/scholar-fetch");
    ...
    function test_a_stranger_cannot_write_a_record_that_reads_as_pactras() public {
        (, uint256 refusalId,) = _draw(opA, childA, aisa, Fixtures.TRANCHE6 + 1);
        record.attest(refusalId);
        ...
        reputation.giveFeedback(
            agentA, 100, 0, "pactra.refused", "window-budget", "", "https://pactra.example/refusal/1", bytes32(0)
        );
    ...
    function test_a_node_cannot_be_rebound_to_a_friendlier_identity_after_a_refusal() public {
        ...
        vm.prank(opA);
        uint256 fresh = identity.register("https://pactra.example/agent/research-worker-take-two");
