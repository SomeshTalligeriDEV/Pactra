...
contract G4_Search is Test {
    ...
    function setUp() public {
        VARIANTS = vm.envOr("PACTRA_G4_VARIANTS", uint256(120));

        for (uint256 i = 0; i < payees.length; ++i) {
            payees[i] = address(uint160(uint256(keccak256(abi.encode("pactra:payee", i)))));
        }
    ...
    function _spawnWider(uint256 seed) private {
        ...
        address op = address(uint160(uint256(keccak256(abi.encode("pactra:wide", seed)))));
    ...
    function _spawn(bytes32 parent, uint128 budget6, uint128 tranche6) private returns (bytes32 child) {
        ...
        address op = address(uint160(uint256(keccak256(abi.encode("pactra:op", parent, nodes.length, strategies)))));
    ...
    function _freshRoot() private {
        rootOperator = address(uint160(uint256(keccak256(abi.encode("pactra:root-op", strategies)))));
