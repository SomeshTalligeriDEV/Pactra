 ...
 *     --rpc-url $ARC_RPC --account pactra-deployer --broadcast
 */
contract Deploy is Script {
    function run() external {
        address usdc = vm.envOr("PACTRA_USDC", Fixtures.USDC);
        address gateway = vm.envOr("PACTRA_GATEWAY", Fixtures.GATEWAY_WALLET);
        address identity = vm.envOr("PACTRA_IDENTITY", Fixtures.IDENTITY_REGISTRY);
        address reputation = vm.envOr("PACTRA_REPUTATION", Fixtures.REPUTATION_REGISTRY);
