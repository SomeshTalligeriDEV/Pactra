...
#     PACTRA_OWNER_ACCOUNT=pactra-owner ./script/open-mandate.sh 0x<rootOperator>
#
...
if [ -z "${PACTRA_OWNER_ACCOUNT:-}" ]; then
  echo "PACTRA_OWNER_ACCOUNT is not set, so nothing was sent."
  echo "Send the calldata above to $REGISTRY from the owner's wallet, or rerun"
  echo "with PACTRA_OWNER_ACCOUNT=<foundry keystore name>."
  exit 0
fi
...
PASSWORD=()
if [ -n "${PACTRA_OWNER_PASSWORD_FILE:-}" ]; then
  PASSWORD=(--password-file "$PACTRA_OWNER_PASSWORD_FILE")
fi

cast send "$REGISTRY" "$SIG" "$ARG" \
  --rpc-url "$RPC" \
  --account "$PACTRA_OWNER_ACCOUNT" \
  ${PASSWORD[@]+"${PASSWORD[@]}"}
