...
import { usePactraReducedMotion } from "./useReducedMotion";

/* ==========================================================================
   Pointer-reactive surfaces.

   A Pactra surface knows where the cursor is over it. That single fact drives
...
export interface PointerField {
  ...
  /** 0–1 across the box, for CSS. Written as `--pactra-px` / `--pactra-py`. */
...
export function usePointerField({ pull = 0, disabled = false }: PointerFieldOptions = {}): PointerField {
  ...
  const onPointerMove = useCallback(
    (event: React.PointerEvent<HTMLElement>) => {
      ...
      node.style.setProperty("--pactra-px", `${(px * 100).toFixed(2)}%`);
      node.style.setProperty("--pactra-py", `${(py * 100).toFixed(2)}%`);
