...
/* ==========================================================================
   ...
   nothing. Pactra keeps the geometry exactly — centre 163,163; ticks every 5°
...
export function Gauge({
  ...
  return (
    <div className={cx("pactra-gauge", className)} style={{ width: size }}>
      <svg
        className="pactra-gauge__svg"
        viewBox="0 0 326 186"
      ...
      >
        <defs>
          <linearGradient id={`pactraArc-${uid}`} gradientUnits="userSpaceOnUse" x1="7" y1="136" x2="312" y2="109">
          ...
          <linearGradient id={`pactraArcShadow-${uid}`} gradientUnits="userSpaceOnUse" x1="11" y1="136" x2="308" y2="110">
          ...
          <radialGradient id={`pactraBeam-${uid}`} gradientUnits="userSpaceOnUse" cx={CENTRE} cy={CENTRE} r="145">
          ...
          <filter id={`pactraGaugeSoft-${uid}`} x="-30%" y="-30%" width="160%" height="160%">
            <feGaussianBlur stdDeviation="1.35" />
          </filter>
          <filter id={`pactraGaugeBloom-${uid}`} x="-40%" y="-40%" width="180%" height="180%">
            <feGaussianBlur stdDeviation="11" />
          </filter>
        </defs>

        {/* the ring: shadow under, lit arc over, hairline inside */}
        <path d={arcPath(START, END, 154)} fill="none" stroke={`url(#pactraArcShadow-${uid})`} strokeWidth="3.2" strokeLinecap="round" />
        <path d={arcPath(START, END, 158.5)} fill="none" className="pactra-gauge__ring" strokeWidth="2.2" strokeLinecap="round" />
        <motion.path
          d={arcPath(START, END, 158.5)}
          fill="none"
          stroke={`url(#pactraArc-${uid})`}
          strokeWidth="2.2"
          strokeLinecap="round"
          pathLength={1}
        ...
        />
        <path d={arcPath(START + 6, END - 40, 146)} fill="none" className="pactra-gauge__hairline" strokeWidth="1.15" />
        ...
        <motion.g
          className={cx(sweep && !reducedMotion && "pactra-gauge__sweeping")}
        ...
        >
          <path d={wedge} fill={`url(#pactraBeam-${uid})`} filter={`url(#pactraGaugeSoft-${uid})`} />
          <path
            d={`M${polar(0, 150).x.toFixed(1)} ${polar(0, 150).y.toFixed(1)}L${polar(0, 55).x.toFixed(1)} ${polar(0, 55).y.toFixed(1)}`}
            className="pactra-gauge__pointer"
            strokeWidth="1.6"
            strokeLinecap="round"
            filter={`url(#pactraGaugeSoft-${uid})`}
          />
        </motion.g>
          ...
          {Array.from({ length: TICK_COUNT }, (_, index) => {
            ...
            return (
              <line
                ...
                className={cx("pactra-gauge__tick", lit && "pactra-gauge__tick--lit")}
              />
            );
          })}
        </g>

        <ellipse cx="225" cy="166" rx="92" ry="76" fill="#fff" opacity=".055" filter={`url(#pactraGaugeBloom-${uid})`} />
      </svg>

      {children || footnote ? (
        <div className="pactra-gauge__readout">
          {children}
          {footnote ? <span className="pactra-gauge__footnote">{footnote}</span> : null}
        </div>
      ) : null}
    </div>
  );
