...
export function Sheet({
  ...
  return createPortal(
    <AnimatePresence>
      {open ? (
        <div className="pactra-sheet-layer">
          <motion.div
          ...
          <motion.div
            ...
            tabIndex={-1}
            className={cx("pactra-sheet", `pactra-sheet--${side}`, className)}
          ...
          >
            <div className="pactra-sheet__grip" aria-hidden="true" />
            <header className="pactra-sheet__header">
              <div>
                {title ? <h2 className="pactra-sheet__title">{title}</h2> : null}
                {description ? <p className="pactra-sheet__description">{description}</p> : null}
              </div>
              <button type="button" className="pactra-modal__close" aria-label="Close" onClick={onClose}>
                <Icon name="close" />
              </button>
            </header>
            <div className="pactra-sheet__body">{children}</div>
            {footer ? <footer className="pactra-sheet__footer">{footer}</footer> : null}
          </motion.div>
        </div>
      ) : null}
    </AnimatePresence>,
