...
export function Carousel({
  ...
  return (
    <section
      className={cx("pactra-carousel", className)}
    ...
    >
      <div
        ref={trackRef}
        className="pactra-carousel__track"
        style={{ "--pactra-carousel-per-view": perView } as React.CSSProperties}
        onScroll={onScroll}
      >
        {children.map((child, index) => (
          <div
            key={index}
            className="pactra-carousel__slide"
            role="group"
      ...
      {showArrows && pages > 1 ? (
        <>
          <button
            type="button"
            className="pactra-carousel__arrow pactra-carousel__arrow--prev"
          ...
          <button
            type="button"
            className="pactra-carousel__arrow pactra-carousel__arrow--next"
      ...
      {showDots && pages > 1 ? (
        <div className="pactra-carousel__dots">
          {Array.from({ length: pages }, (_, index) => (
            <button
              key={index}
              type="button"
              className={cx("pactra-carousel__dot", index === active && "pactra-carousel__dot--active")}
