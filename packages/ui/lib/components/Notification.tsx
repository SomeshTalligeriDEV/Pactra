...
export function Notification({
  ...
  return (
    <div
      className={cx("pactra-notification", `pactra-notification--${tone}`, className)}
      role={tone === "critical" ? "alert" : "status"}
    >
      {!hideIcon ? (
        <span className="pactra-notification__icon" aria-hidden="true">
          <Icon name={TONE_ICON[tone]} />
        </span>
      ) : null}
      <div className="pactra-notification__text">
        {title ? <p className="pactra-notification__title">{title}</p> : null}
        {children ? <div className="pactra-notification__body">{children}</div> : null}
      </div>
      {action ? <div className="pactra-notification__action">{action}</div> : null}
      {onDismiss ? (
        <button type="button" className="pactra-notification__close" aria-label="Dismiss" onClick={onDismiss}>
          <Icon name="close" />
        </button>
      ) : null}
    </div>
...
export function ToastProvider({ children, placement = "bottom-right" }: ToastProviderProps) {
  ...
  const notify = useCallback<ToastContextValue["notify"]>(
    (toast) => {
      const id = toast.id ?? `pactra-toast-${Math.random().toString(36).slice(2, 9)}`;
  ...
  return (
    <ToastActionsContext.Provider value={actions}>
      ...
      {typeof document !== "undefined"
        ? createPortal(
            <div className={cx("pactra-toasts", `pactra-toasts--${placement}`)} aria-live="polite">
              ...
              <AnimatePresence initial={false}>
                {toasts.map((toast) => (
                  ...
                  >
                    <Notification
                      {...toast}
                      className="pactra-notification--toast"
