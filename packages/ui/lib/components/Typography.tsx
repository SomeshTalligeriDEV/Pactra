...
export function Headline({ lines, dotWord, as, animate = true, className, ...rest }: HeadlineProps) {
  ...
  const renderLine = (line: ReactNode) => {
    ...
    return (
      <>
        {before}
        <DotText className="pactra-headline__dotword" spacing="word" radius={1.8}>
          {dotWord}
        </DotText>
        {after}
      </>
  ...
  return (
    <Tag className={cx("pactra-headline", className)} {...rest}>
      {lines.map((line, index) => (
        <motion.span
          key={index}
          className="pactra-headline__line"
...
export function Text({ variant = "body", tone = "copy", as, animate = false, className, children, ...rest }: TextProps) {
  ...
  return (
    <Component
      className={cx("pactra-text", `pactra-text--${variant}`, `pactra-text--${tone}`, className)}
...
/** Glossary: "Logo". */
export function Logo({ name = "Pactra", markOnly = false, size = 22, className, ...rest }: LogoProps) {
  return (
    <span className={cx("pactra-logo", className)} style={{ fontSize: size }} {...rest}>
      <span className="pactra-logo__mark" aria-hidden="true">
        <svg viewBox="0 0 24 24" fill="none">
          <rect x="2.5" y="2.5" width="19" height="19" rx="5.5" fill="url(#pactraLogoGlaze)" />
          ...
          <defs>
            <linearGradient id="pactraLogoGlaze" x1="2" y1="2" x2="22" y2="22">
      ...
      {!markOnly ? (
        <DotText className="pactra-logo__word" spacing="word" radius={1.8}>
          {name}
        </DotText>
      ) : null}
      <span className="pactra-visually-hidden">{name}</span>
    </span>
  );
}
