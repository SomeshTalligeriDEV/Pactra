...
export const Card = forwardRef<HTMLDivElement, CardProps>(function Card(
  ...
  return (
    <motion.div
      ref={ref}
      className={cx("pactra-card", className)}
    ...
    >
      <Surface
        ...
        className="pactra-card__surface"
      >
        {children}
      </Surface>
    </motion.div>
...
export function CardMedia({ src, alt = "", ratio = "16/9", className, style, children, ...rest }: CardMediaProps) {
  return (
    <div className={cx("pactra-card__media", className)} style={{ aspectRatio: ratio, ...style }} {...rest}>
...
export function CardHeader({ className, children, ...rest }: HTMLAttributes<HTMLDivElement>) {
  return (
    <div className={cx("pactra-card__header", className)} {...rest}>
      {children}
    </div>
  );
}
...
export function CardBody({ className, children, ...rest }: HTMLAttributes<HTMLDivElement>) {
  return (
    <div className={cx("pactra-card__body", className)} {...rest}>
      {children}
    </div>
  );
}
...
export function CardFooter({ className, children, ...rest }: HTMLAttributes<HTMLDivElement>) {
  return (
    <div className={cx("pactra-card__footer", className)} {...rest}>
      {children}
    </div>
  );
}
...
export function MetricCard({
  ...
  return (
    <motion.article
      ...
      className={cx("pactra-metriccard", !action && "pactra-metriccard--bare", className)}
    ...
    >
      <Surface glaze={glaze} radius="tile" elevation="tile" className="pactra-metriccard__surface">
        <motion.h3 className="pactra-metriccard__title" {...child(CHILD_OFFSET.title)}>
          {title}
        </motion.h3>

        {figure !== null ? (
          <motion.div className="pactra-metriccard__figure" {...child(CHILD_OFFSET.figure)}>
            {figure ?? <Gauge value={progress} size="100%" />}
          </motion.div>
        ) : null}

        <motion.div className="pactra-metriccard__metric" {...child(CHILD_OFFSET.metric)}>
          <DotText className="pactra-metriccard__value" radius={2.05}>
            {value}
          </DotText>
          {unit ? <span className="pactra-metriccard__unit">{unit}</span> : null}
        </motion.div>

        {caption ? (
          <motion.p className="pactra-metriccard__caption" {...child(CHILD_OFFSET.caption)}>
            {caption}
          </motion.p>
        ) : null}

        {action ? (
          <motion.div className="pactra-metriccard__action" {...child(CHILD_OFFSET.control)}>
