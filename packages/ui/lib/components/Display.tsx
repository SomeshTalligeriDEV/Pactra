...
export function Segmented({
  ...
  return (
    <div
      role="radiogroup"
      aria-label={label}
      className={cx("pactra-segmented", `pactra-segmented--${size}`, block && "pactra-segmented--block", className)}
    >
      {options.map((option) => {
        ...
        return (
          <button
            ...
            className={cx("pactra-segmented__option", active && "pactra-segmented__option--active")}
            onClick={() => {
          ...
          >
            {active ? (
              <motion.span
                layoutId={`pactra-segmented-${groupId}`}
                className="pactra-segmented__thumb"
...
export function Avatar({ src, name = "", size = "md", status = "none", className, ...rest }: AvatarProps) {
  ...
  return (
    <span
      className={cx("pactra-avatar", `pactra-avatar--${size}`, status !== "none" && `pactra-avatar--${status}`, className)}
      title={name || undefined}
      {...rest}
    >
      {src ? <img src={src} alt={name} /> : <span aria-hidden="true">{initials || "?"}</span>}
      {!src && name ? <span className="pactra-visually-hidden">{name}</span> : null}
    </span>
  );
}
...
export function AvatarGroup({ children, max, total, className }: AvatarGroupProps) {
  ...
  return (
    <span className={cx("pactra-avatar-group", className)}>
      {shown}
      {hidden > 0 ? <span className="pactra-avatar pactra-avatar--md pactra-avatar--more">+{hidden}</span> : null}
    </span>
  );
}
...
export function Skeleton({ variant = "block", lines = 3, width, height, className, style, ...rest }: SkeletonProps) {
  if (variant === "text") {
    return (
      <span className={cx("pactra-skeleton-text", className)} aria-hidden="true" {...rest}>
        {Array.from({ length: lines }, (_, index) => (
          <span
            key={index}
            className="pactra-skeleton"
  ...
  return (
    <span
      className={cx("pactra-skeleton", variant === "circle" && "pactra-skeleton--circle", className)}
...
export function EmptyState({ icon = "layers", title, description, action, ornament = true, className }: EmptyStateProps) {
  return (
    <div className={cx("pactra-empty", className)}>
      <span className={cx("pactra-empty__mark", ornament && "pactra-empty__mark--ring")} aria-hidden="true">
        <Icon name={icon} />
      </span>
      <p className="pactra-empty__title">{title}</p>
      {description ? <p className="pactra-empty__description">{description}</p> : null}
      {action ? <div className="pactra-empty__action">{action}</div> : null}
    </div>
  );
}
