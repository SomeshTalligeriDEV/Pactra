...
export function Bento({ columns = 4, rowHeight = 160, gap = "md", className, style, children, ...rest }: BentoProps) {
  return (
    <div
      className={cx("pactra-bento", `pactra-bento--gap-${gap}`, className)}
      style={{
        "--pactra-bento-columns": columns,
        "--pactra-bento-row": typeof rowHeight === "number" ? `${rowHeight}px` : rowHeight,
        ...style,
      } as CSSProperties}
      {...rest}
    >
...
export function BentoCell({
  ...
  return (
    <motion.div
      className={cx("pactra-bento__cell", className)}
    ...
    >
      <Surface
        ...
        className="pactra-bento__surface"
      >
        {children}
      </Surface>
    </motion.div>
