...
/* ==========================================================================
   ...
   The glossary separates them, and so does Pactra: a Loader answers "is it
...
export function ProgressBar({
  ...
  return (
    <div className={cx("pactra-progress", `pactra-progress--${size}`, className)} data-glaze={glaze ?? houseGlaze}>
      {label || showValue ? (
        <div className="pactra-progress__header">
          {label ? <span className="pactra-progress__label">{label}</span> : null}
          {showValue && !indeterminate ? (
            <span className="pactra-progress__value">{Math.round(clamped)}%</span>
          ) : null}
        </div>
      ) : null}
      <div
        className="pactra-progress__track"
        role="progressbar"
      ...
      >
        {indeterminate ? (
          <span className="pactra-progress__fill pactra-progress__fill--sweep" />
        ) : (
          <motion.span
            className="pactra-progress__fill"
            initial={false}
...
export function Loader({ variant = "radar", size = "md", label = "Loading", className }: LoaderProps) {
  ...
  return (
    <span
      className={cx("pactra-loader", `pactra-loader--${variant}`, `pactra-loader--${size}`, className)}
      role="status"
      aria-live="polite"
      aria-label={label}
    >
      {variant === "radar" ? (
        <span className="pactra-loader__radar" aria-hidden="true">
          <svg viewBox="0 0 100 100">
            <circle className="pactra-loader__radar-ring" cx="50" cy="50" r="42" />
            {Array.from({ length: 24 }, (_, index) => {
              ...
              return (
                <line
                  ...
                  className="pactra-loader__radar-tick"
                  strokeWidth={major ? 1.6 : 1}
                />
              );
            })}
            <g className={reducedMotion ? undefined : "pactra-loader__radar-sweep"}>
        ...
        <Icon name="spinner" className="pactra-loader__spinner" />
      ) : variant === "dots" ? (
        <span className="pactra-loader__dots" aria-hidden="true">
          {[0, 1, 2].map((i) => (
            <motion.span
              key={i}
              className="pactra-loader__dot"
        ...
        <span className="pactra-loader__bar" aria-hidden="true">
          <span className="pactra-loader__bar-fill" />
        </span>
      )}
    </span>
  );
...
export function StepProgress({ steps, current, orientation = "horizontal", className }: StepProgressProps) {
  ...
  const rail = (lit: boolean) => {
    ...
    d += `L${width} ${RAIL_Y}`;
    return <path d={d} className={lit ? "pactra-steps__rail-lit" : "pactra-steps__rail-base"} />;
  };

  const travelled = count > 1 ? Math.min(1, Math.max(0, (current + 0.5) / count)) : 1;

  return (
    <div className={cx("pactra-steps", `pactra-steps--${orientation}`, className)}>
      {orientation === "horizontal" ? (
        <div ref={railRef} className="pactra-steps__map">
          {railSize.width ? (
      ...
      <ol className="pactra-steps__list">
        {steps.map((step, index) => {
          const state = index < current ? "done" : index === current ? "current" : "todo";
          return (
            <li key={index} className={cx("pactra-steps__item", `pactra-steps__item--${state}`)}>
              <span className="pactra-steps__marker" aria-hidden="true">
                {state === "done" ? <Icon name="check" /> : index + 1}
              </span>
              <span className="pactra-steps__text">
                <span className="pactra-steps__label">{step.label}</span>
                {step.description ? (
                  <span className="pactra-steps__description">{step.description}</span>
                ) : null}
              </span>
              <span className="pactra-visually-hidden">
