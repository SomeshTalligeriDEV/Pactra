...
/* ==========================================================================
   ...
   2020". Pactra's is a glass panel — the same four-layer recipe as every
...
export function CommandPalette({
  ...
  return createPortal(
    <AnimatePresence>
      {open ? (
        <div className="pactra-command-layer">
          <motion.div
          ...
          <motion.div
            ...
            onKeyDown={onKeyDown}
          >
            <div className="pactra-command__search">
              <Icon name="search" className="pactra-command__search-icon" />
              <input
                ref={inputRef}
                className="pactra-command__input"
                value={query}
            ...
            <div ref={listRef} className="pactra-command__list" role="listbox">
              {rows.length ? (
                rows.map((row, position) =>
                  row.kind === "heading" ? (
                    <p key={`h-${position}`} className="pactra-command__heading">
                      {row.label}
                    </p>
                  ) : (
                    <button
                      ...
                      className={cx(
                        "pactra-command__item",
                        row.index === active && "pactra-command__item--active",
                      )}
                      onMouseMove={() => setActive(row.index)}
                      onClick={() => run(row.item)}
                    >
                      {row.item.icon ? (
                        <Icon name={row.item.icon} className="pactra-command__item-icon" />
                      ) : (
                        <span className="pactra-command__item-icon" />
                      )}
                      <span className="pactra-command__item-label">{row.item.label}</span>
                      {row.item.shortcut ? <Kbd keys={row.item.shortcut} /> : null}
                      {row.index === active ? (
                        <motion.span
                          layoutId="pactra-command-marker"
                          className="pactra-command__marker"
                ...
                <div className="pactra-command__empty">
                  {empty ?? (
                    <>
                      <Icon name="search" />
            ...
            <footer className="pactra-command__footer">
              <span><Kbd keys={["↑", "↓"]} /> navigate</span>
              <span><Kbd keys={["↵"]} /> run</span>
              <span className="pactra-command__count">
