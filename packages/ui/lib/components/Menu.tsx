...
export function Menu({ items, trigger, align = "start", side = "bottom", width, className, onOpenChange }: MenuProps) {
  ...
  const menuId = `pactra-menu-${useId().replace(/[:»«]/g, "")}`;
  ...
  return (
    <div ref={rootRef} className={cx("pactra-menu", className)}>
      <span
        className="pactra-menu__trigger"
      ...
      <AnimatePresence>
        {open ? (
          <motion.div
            id={menuId}
            role="menu"
            className={cx("pactra-menu__panel", `pactra-menu__panel--${align}`, `pactra-menu__panel--${side}`)}
          ...
          >
            {items.map((item, index) => {
              if (item === "separator") {
                return <span key={`sep-${index}`} className="pactra-menu__separator" role="separator" />;
              }
              ...
              return (
                <button
                  ...
                  className={cx(
                    "pactra-menu__item",
                    item.destructive && "pactra-menu__item--destructive",
                    position === activeIndex && "pactra-menu__item--active",
                  )}
                ...
                >
                  {item.icon ? <Icon name={item.icon} className="pactra-menu__icon" /> : null}
                  <span className="pactra-menu__label">{item.label}</span>
                  {item.meta ? <span className="pactra-menu__meta">{item.meta}</span> : null}
                </button>
              );
            })}
          </motion.div>
...
export function Sidebar({ sections, value, onValueChange, header, footer, collapsed = false, className }: SidebarProps) {
  return (
    <nav
      className={cx("pactra-sidebar", collapsed && "pactra-sidebar--collapsed", className)}
      aria-label="Sidebar"
    >
      {header ? <div className="pactra-sidebar__header">{header}</div> : null}

      <div className="pactra-sidebar__scroll">
        {sections.map((section, index) => (
          <div key={index} className="pactra-sidebar__section">
            {section.title && !collapsed ? (
              <p className="pactra-sidebar__section-title">{section.title}</p>
            ) : null}
            <ul className="pactra-sidebar__list">
              {section.items.map((item) => (
                <li key={item.id}>
                  <button
                    ...
                    className={cx("pactra-sidebar__item", item.id === value && "pactra-sidebar__item--active")}
                    onClick={() => onValueChange?.(item.id)}
                    title={collapsed && typeof item.label === "string" ? item.label : undefined}
                  >
                    {item.icon ? <Icon name={item.icon} className="pactra-sidebar__icon" /> : null}
                    {!collapsed ? <span className="pactra-sidebar__label">{item.label}</span> : null}
                    {item.badge && !collapsed ? (
                      <span className="pactra-sidebar__badge">{item.badge}</span>
                    ) : null}
                  </button>
                </li>
              ))}
      ...
      {footer ? <div className="pactra-sidebar__footer">{footer}</div> : null}
    </nav>
  );
}
...
export function Drawer({ open, onClose, side = "left", title, children, className }: DrawerProps) {
  return (
    <AnimatePresence>
      {open ? (
        <div className="pactra-drawer-layer">
          <motion.div
            className="pactra-scrim"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            onClick={onClose}
          />
          <motion.aside
            className={cx("pactra-drawer", `pactra-drawer--${side}`, className)}
          ...
          >
            <div className="pactra-drawer__header">
              {title ? <h2 className="pactra-drawer__title">{title}</h2> : <span />}
              <button type="button" className="pactra-modal__close" aria-label="Close" onClick={onClose}>
                <Icon name="close" />
              </button>
            </div>
            <div className="pactra-drawer__body">{children}</div>
          </motion.aside>
        </div>
      ) : null}
    </AnimatePresence>
