...
export function DataTable<Row>({
  ...
  if (!rows.length) {
    return <div className={cx("pactra-table__shell", className)}>{empty ?? <EmptyState title="Nothing here yet" description="Rows will appear once there is something to show." />}</div>;
  }

  return (
    <div
      className={cx("pactra-table__shell", className)}
      data-pactra-density={density}
      style={{ maxHeight }}
    >
      <table className="pactra-table">
        <thead className={cx(stickyHeader && "pactra-table__head--sticky")}>
          <tr>
            {selectable ? (
              <th scope="col" className="pactra-table__select">
                <Checkbox
              ...
              return (
                ...
                >
                  {column.sortBy ? (
                    <button
                      type="button"
                      className={cx("pactra-table__sort", active && "pactra-table__sort--active")}
                      onClick={() => toggleSort(column)}
                    >
                      <span>{column.header}</span>
                      <span className="pactra-table__sort-mark">
                        {active ? (
                          <motion.span layoutId="pactra-table-sort" transition={spring.snap}>
                            <Icon name={sort!.dir === "asc" ? "chevron-up" : "chevron-down"} />
                          </motion.span>
                        ) : (
                          <Icon name="sort" className="pactra-table__sort-idle" />
                        )}
                      </span>
                    </button>
                  ) : (
        ...
        <tbody>
          {sorted.map((row) => {
            ...
            return (
              <tr
                key={id}
                className={cx(isPicked && "pactra-table__row--selected", onRowClick && "pactra-table__row--clickable")}
                onClick={onRowClick ? () => onRowClick(row) : undefined}
              >
                {selectable ? (
                  <td className="pactra-table__select" onClick={(event) => event.stopPropagation()}>
                    <Checkbox
                ...
                {columns.map((column) => (
                  <td
                    key={column.id}
                    className={cx(column.numeric && "pactra-table__cell--numeric")}
