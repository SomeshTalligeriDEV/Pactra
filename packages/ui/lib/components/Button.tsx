...
export const Button = forwardRef<HTMLButtonElement, ButtonProps>(function Button(
  ...
  return (
    <motion.button
      ...
      className={cx(
        "pactra-button",
        `pactra-button--${variant}`,
        `pactra-button--${size}`,
        iconOnly && "pactra-button--icon-only",
        block && "pactra-button--block",
        loading && "pactra-button--loading",
        className,
      )}
    ...
    >
      {loading ? (
        <Icon name="spinner" className="pactra-button__spinner" />
      ) : (
        iconStart && <Icon name={iconStart} className="pactra-button__icon" />
      )}
      {!iconOnly && children ? <span className="pactra-button__label">{children}</span> : null}
      {!loading && iconEnd ? <Icon name={iconEnd} className="pactra-button__icon" /> : null}
    </motion.button>
  );
});
...
export const Cta = forwardRef<HTMLButtonElement, CtaProps>(function Cta(
  ...
  return (
    <Button
      ...
      size="lg"
      iconEnd={iconEnd}
      className={cx("pactra-cta", hint && "pactra-cta--stacked", className)}
      {...rest}
    >
      <span className="pactra-cta__label">{children}</span>
      {hint ? <span className="pactra-cta__hint">{hint}</span> : null}
    </Button>
  );
});
  ...
  function PlusButton({ label = "Add", className, ...rest }, ref) {
    return (
      <Button
        ...
        iconStart="plus"
        aria-label={label}
        className={cx("pactra-button--plus", className)}
        {...rest}
      />
    );
  },
