...
/* ==========================================================================
   ...
   off the bottom edge. Pactra uses it as a layout. Cards do not float above a
...
export function TileWall({
  ...
  return (
    <div
      className={cx("pactra-wall", deep && "pactra-wall--deep", className)}
      data-glaze={glaze}
      style={{
        "--pactra-wall-columns": columns,
        "--pactra-wall-grout": `${grout}px`,
        ...style,
      } as React.CSSProperties}
      {...rest}
    >
      <span className="pactra-wall__bloom" aria-hidden="true" />
      {children}
    </div>
  );
}
...
export function Tile({ span = 1, rows = 1, className, style, children, ...rest }: TileProps) {
  return (
    <div
      className={cx("pactra-wall__tile", className)}
