...
export function Calendar({ value, onValueChange, min, max, locale = "en-GB", className }: CalendarProps) {
  ...
  return (
    <div className={cx("pactra-calendar", className)}>
      <div className="pactra-calendar__header">
        <button
          type="button"
          className="pactra-calendar__nav"
          aria-label="Previous month"
          onClick={() => setMonth(new Date(month.getFullYear(), month.getMonth() - 1, 1))}
        >
          <Icon name="chevron-left" />
        </button>
        <span className="pactra-calendar__heading" aria-live="polite">
          {heading}
        </span>
        <button
          type="button"
      ...
      <div className="pactra-calendar__weekdays" aria-hidden="true">
        {DAY_LABELS.map((day) => (
          <span key={day}>{day}</span>
        ))}
      </div>

      <div className="pactra-calendar__grid" role="grid">
        {cells.map((date, index) =>
          date ? (
            <button
              ...
              className={cx(
                "pactra-calendar__day",
                sameDay(date, value) && "pactra-calendar__day--selected",
                sameDay(date, today) && "pactra-calendar__day--today",
              )}
            ...
            <span key={index} className="pactra-calendar__spacer" />
          ),
        )}
      </div>
    </div>
...
export function DatePicker({
  ...
  return (
    <div className={cx("pactra-datepicker", className)}>
      {label ? <span className="pactra-field__label">{label}</span> : null}
      <button
        type="button"
        className={cx("pactra-input", "pactra-input--md", "pactra-datepicker__trigger", open && "pactra-input--focused", disabled && "pactra-input--disabled")}
        aria-haspopup="dialog"
        aria-expanded={open}
        disabled={disabled}
        onClick={() => setOpen((state) => !state)}
      >
        <Icon name="calendar" className="pactra-input__icon" />
        <span className={cx("pactra-select__value", !value && "pactra-select__value--placeholder")}>{printed}</span>
      </button>
      ...
      <AnimatePresence>
        {open ? (
          <motion.div
            ...
            className="pactra-datepicker__panel"
          ...
          >
            ...
            />
            {withTime ? (
              <div className="pactra-datepicker__time">
                <Icon name="clock" />
                <input
                  type="time"
                  className="pactra-datepicker__time-input"
                  aria-label="Time"
                  value={
                    value
                ...
                />
                <button type="button" className="pactra-datepicker__done" onClick={() => setOpen(false)}>
                  Done
                </button>
              </div>
            ) : null}
