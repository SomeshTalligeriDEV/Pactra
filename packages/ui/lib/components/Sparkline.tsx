...
export function Sparkline({
  ...
  return (
    <div
      ref={ref}
      className={cx("pactra-sparkline", className)}
      style={{ width, height }}
      data-glaze={firing}
    ...
    >
      {geometry ? (
        <svg width={size.width} height={height} aria-hidden="true">
          <defs>
            <linearGradient id={`pactraSpark-${uid}`} x1="0" y1="0" x2="0" y2="1">
          ...
          {area ? <path d={geometry.fill} fill={`url(#pactraSpark-${uid})`} /> : null}
          ...
          {extremes && !geometry.flat ? (
            <>
              <circle cx={geometry.highest.x} cy={geometry.highest.y} r="2.6" fill="var(--pactra-paper)" stroke="currentColor" strokeWidth="1.2" />
              <circle cx={geometry.lowest.x} cy={geometry.lowest.y} r="2.6" fill="var(--pactra-paper)" stroke="currentColor" strokeWidth="1.2" opacity=".55" />
            </>
          ) : null}
