...
import { usePactraId } from "../../hooks/useId";
import { cx } from "../cx";
...
export function Dropzone({
  ...
  return (
    <div className={cx("pactra-dropzone-wrap", className)}>
      <div
        className={cx("pactra-dropzone", over && "pactra-dropzone--over", disabled && "pactra-dropzone--disabled")}
        onDragEnter={(event) => {
      ...
      >
        <input
          ...
          id={id}
          type="file"
          className="pactra-visually-hidden"
          accept={accept}
          multiple={multiple}
          disabled={disabled}
        ...
        />
        <motion.span
          className="pactra-dropzone__mark"
        ...
        <p className="pactra-dropzone__title">
          {over ? "Drop to upload" : "Drag files here"}
        </p>
        <p className="pactra-dropzone__hint">{hint ?? (accept ? `${accept} · ` : "") }
          <button type="button" className="pactra-dropzone__browse" onClick={() => inputRef.current?.click()} disabled={disabled}>
            browse
          </button>
        </p>
        {rejected ? <p className="pactra-dropzone__error">{rejected}</p> : null}
      </div>

      {files.length ? (
        <ul className="pactra-dropzone__list">
          <AnimatePresence initial={false}>
            {files.map((entry) => (
              <motion.li
                ...
                exit={{ opacity: 0, x: 16 }}
                transition={spring.snap}
                className="pactra-dropzone__file"
              >
                <Icon name={entry.file.type.startsWith("image") ? "image" : "layers"} />
                <span className="pactra-dropzone__file-name">{entry.file.name}</span>
                {entry.progress !== undefined ? (
                  <span className="pactra-dropzone__file-bar">
                    <span style={{ width: `${entry.progress}%` }} />
                  </span>
                ) : entry.error ? (
                  <span className="pactra-dropzone__file-error">{entry.error}</span>
                ) : (
                  <Icon name="check-circle" className="pactra-dropzone__file-done" />
                )}
                {onRemove ? (
...
export function OtpInput({
  ...
  return (
    <div className={cx("pactra-otp", invalid && "pactra-otp--invalid", className)} role="group" aria-label={label}>
      {Array.from({ length }, (_, index) => (
        <span key={index} className={cx("pactra-otp__box", current[index] && "pactra-otp__box--filled")}>
          {dots && current[index] ? (
            <DotText inline radius={1.75} className="pactra-otp__dots" aria-hidden="true">
              {current[index]}
            </DotText>
          ) : null}
          <input
            ref={(node) => { refs.current[index] = node; }}
            className="pactra-otp__input"
            inputMode="numeric"
...
export function Resizable({ start, end, defaultSplit = 0.5, min = 0.2, max = 0.8, className }: ResizableProps) {
  ...
  return (
    <div ref={rootRef} className={cx("pactra-resizable", className)} style={{ "--pactra-split": `${split * 100}%` } as React.CSSProperties}>
      <div className="pactra-resizable__pane">{start}</div>
      <div
        role="separator"
        ...
        tabIndex={0}
        className="pactra-resizable__handle"
        onPointerDown={(event) => {
      ...
      >
        <span className="pactra-resizable__grip" />
      </div>
      <div className="pactra-resizable__pane">{end}</div>
    </div>
  );
}
