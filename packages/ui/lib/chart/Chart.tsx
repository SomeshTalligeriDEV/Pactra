...
export function LineChart({
  ...
  const { reducedMotion, glaze: houseGlaze } = usePactra();
  ...
  return (
    <div
      ref={ref}
      className={cx("pactra-chart", className)}
      style={{ height }}
      role="img"
    ...
    >
        ...
        >
          ...
          <defs>
            {model.lines.map((line) => {
              ...
              return (
                <linearGradient key={line.series.id} id={`pactraLine-${uid}-${line.series.id}`} x1="0" y1="0" x2="0" y2="1">
          ...
          {/* structure */}
          <g className="pactra-chart__grid">
            {model.ticks.values.map((value) => (
              <g key={value}>
                <line x1={pad.left} x2={size.width - pad.right} y1={model.y(value)} y2={model.y(value)} />
                <text x={pad.left - 8} y={model.y(value)} dy="0.32em" textAnchor="end" className="pactra-chart__axis">
                  {format(value)}
                </text>
              </g>
            ))}
          ...
          {active !== null ? (
            <line
              className="pactra-chart__crosshair"
              x1={model.x(active)}
              x2={model.x(active)}
              y1={pad.top}
            ...
            return (
              <g key={line.series.id} style={{ color: ink }}>
                {area && !line.series.reference ? (
                  <path d={line.fill} fill={`url(#pactraLine-${uid}-${line.series.id})`} />
                ) : null}
                <motion.path
                  d={line.d}
                  fill="none"
                ...
                {active !== null && line.points[active] && !line.series.reference ? (
                  <motion.circle
                    layoutId={`pactra-chart-dot-${uid}-${line.series.id}`}
                    cx={line.points[active].x}
                    cy={line.points[active].y}
                    r="4"
                    fill="var(--pactra-paper)"
                    stroke="currentColor"
                    strokeWidth="2"
      ...
      {active !== null && model ? (
        <div
          className="pactra-chart__readout"
          style={{
            left: Math.min(Math.max(model.x(active), 64), size.width - 64),
          }}
        >
          {labels?.[active] ? <span className="pactra-chart__readout-label">{labels[active]}</span> : null}
          ...
          {series
            ...
            .map((s) => (
              <span key={s.id} className="pactra-chart__readout-row">
                <span className="pactra-chart__swatch" style={{ background: GLAZE_INK[s.glaze ?? houseGlaze] }} />
                {s.label ? <span className="pactra-chart__readout-name">{s.label}</span> : null}
                <span className="pactra-chart__readout-value">{format(s.values[active] ?? 0)}</span>
...
export function BarChart({
  ...
  return (
    <div ref={ref} className={cx("pactra-chart", className)} style={{ height }} role="img" aria-label={label ?? "Bar chart"}>
      {model ? (
          ...
          {data.map((datum, index) => {
            ...
            if (horizontal) {
              ...
              return (
                <g key={`${datum.label}-${index}`} onPointerEnter={() => setHover(index)} onPointerLeave={() => setHover(null)}>
                  <text x={pad.left - 10} y={y + model.thickness / 2} dy="0.32em" textAnchor="end" className="pactra-chart__axis">
                    {datum.label}
                  </text>
                  <motion.rect
                    x={pad.left}
                  ...
                  />
                  {showValues ? (
                    <text x={pad.left + length + 7} y={y + model.thickness / 2} dy="0.32em" className="pactra-chart__value">
                      {format(datum.value)}
                    </text>
                  ) : null}
                </g>
            ...
            return (
              <g key={`${datum.label}-${index}`} onPointerEnter={() => setHover(index)} onPointerLeave={() => setHover(null)}>
                ...
                />
                {showValues ? (
                  <text x={x + model.thickness / 2} y={top - 7} textAnchor="middle" className="pactra-chart__value">
                    {format(datum.value)}
                  </text>
                ) : null}
                <text x={x + model.thickness / 2} y={height - 6} textAnchor="middle" className="pactra-chart__axis">
                  {datum.label}
                </text>
              </g>
            );
...
export function DonutChart({ data, size = 180, thickness = 0.2, children, label, className }: DonutChartProps) {
  ...
  return (
    <div className={cx("pactra-donut", className)} style={{ width: size, height: size }} role="img" aria-label={label ?? "Donut chart"}>
      ...
      {children ? (
        <div
          className="pactra-donut__centre"
