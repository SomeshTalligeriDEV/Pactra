...
The Pactra design system, vendored as source.
...
Consumed through a Vite alias (`pactra-ui` → `packages/ui/index.ts`), so the
import path reads the same as it would from npm.

```tsx
import { PactraProvider, MetricCard, Enforced } from "pactra-ui";
```
...
| | |
|---|---|
| `lib/primitives` | `Surface` (the glaze, the rim, the grain), `DotText` and the LED numerals, `Gauge`, `AnimatedNumber`, `Grain`, `PactraDefs` |
...
`data-pactra-density="compact"` on a shell tightens the whole system. The docs
