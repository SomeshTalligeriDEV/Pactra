#!/usr/bin/env bash
#
# Rebuilds both static surfaces and publishes them to /var/www/pactra.
#
...
#
# It copies and never deletes at the root. /var/www/pactra also holds
...
#
#   pactra-publish.sh              # rebuild and publish from ~/pactra
#   pactra-publish.sh --dry-run    # say what would change, touch nothing
#
set -euo pipefail

REPO=${PACTRA_REPO:-$HOME/pactra}
WWW=${PACTRA_WWW:-/var/www/pactra}
DRY=no
[ "${1:-}" = "--dry-run" ] && DRY=yes
...
# in PACTRA_WWW names a directory that can be created, written into and served
...
[ -f "$SITE/app.html" ] || refuse "$SITE/app.html is missing; product/docs/record routes need their SPA shell"
[ -f "$SITE/main.js" ] || refuse "$SITE/main.js is missing; the static landing was not copied"
...
log "  https://pactra.example/"
log "  https://pactra.example/console/"
