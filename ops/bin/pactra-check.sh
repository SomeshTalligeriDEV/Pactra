...
#     PACTRA_SITE=http://localhost:5274 ops/bin/pactra-check.sh
set -uo pipefail

SITE="${PACTRA_SITE:-https://pactra.example}"
ATTEST="${PACTRA_ATTEST:-https://attest.pactra.example}"
DEMO_SELLER="${PACTRA_DEMO_SELLER:-https://demo-seller.pactra.example}"
...
echo "site"
for path in "/" "/main.js" "/styles.css" "/product" "/console/" "/docs" "/drill"; do
...
# The home page is deliberately framework-free. Evidence lives in the
# separately built product/docs/record application, whose shell is app.html.
BUNDLE="$(curl -s --max-time 15 "$SITE/app.html" | grep -o 'assets/[^"]*\.js' | head -1)"
if [ -z "$BUNDLE" ]; then
  bad "no product/docs bundle referenced by app.html"
else
...
case "$status" in
  ...
  502|000) pending "attest is not running ($status) — it needs PACTRA_ATTEST_KEY" ;;
...
case "$status" in
  ...
  502|000) pending "demo seller is not running ($status) — pactra-demo-seller reads .env.attest" ;;
