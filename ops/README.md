...
| What | Where | How |
|---|---|---|
| site | `https://pactra.example/` | static, `/var/www/pactra` |
| console | `https://pactra.example/console/` | static, `/var/www/pactra/console`, built with base `/console/` |
| meter read API | `https://pactra.example/api/` | `pactra-meter.service` on `127.0.0.1:8404`, proxied |
| attest | `https://attest.pactra.example/` | `pactra-attest.service` on `127.0.0.1:8405`, proxied |
| demo seller | `https://demo-seller.pactra.example/` | `pactra-demo-seller.service` on `127.0.0.1:8406`, proxied |
...
`ConductRecord` writes `https://pactra.example/refusal/<id>` and the daemon
writes `https://pactra.example/agent/<id>`. Those paths belong to the name in
the record.
...
git clone git@github.com:SomeshTalligeriDEV/pactra.git ~/pactra
sudo mkdir -p /var/www/pactra && sudo chown "$USER:$USER" /var/www/pactra
cd ~/pactra && npm ci --prefix packages/site && npm ci --prefix packages/console

# 2. a server that answers ACME, and the site over plain HTTP
sudo cp ops/nginx/bootstrap.conf /etc/nginx/sites-available/pactra
sudo ln -sf /etc/nginx/sites-available/pactra /etc/nginx/sites-enabled/pactra
...
sudo certbot certonly --webroot -w /var/www/pactra \
  -d pactra.example -d www.pactra.example
sudo certbot certonly --webroot -w /var/www/pactra -d attest.pactra.example

# 4. the real configuration
# Both snippets. `pactra-headers.conf` is included from every `location` in
...
sudo cp ops/nginx/pactra-headers.conf ops/nginx/pactra-locations.conf \
       ops/nginx/pactra-compression.conf /etc/nginx/snippets/
# The rate-limit zone. conf.d, not the vhost: `limit_req_zone` is http-context
# and must be declared exactly once on the box.
sudo cp ops/nginx/pactra-limits.conf /etc/nginx/conf.d/
sudo cp ops/nginx/pactra.example.conf /etc/nginx/sites-available/pactra
sudo cp ops/nginx/attest.pactra.example.conf /etc/nginx/sites-available/pactra-attest
sudo ln -sf /etc/nginx/sites-available/pactra-attest /etc/nginx/sites-enabled/pactra-attest
sudo nginx -t && sudo systemctl reload nginx
...
curl -sI https://pactra.example/console/ | grep -i 'frame-ancestors\|nosniff\|strict-transport'
curl -sI https://pactra.example/assets/ -o /dev/null -w '%{http_code}\n'
curl -sI https://attest.pactra.example/health | grep -i 'nosniff\|strict-transport'
...
curl -sI --compressed https://pactra.example/console/ | grep -i content-encoding
```
...
umask 077 && ${EDITOR:-nano} ~/pactra/.env.attest    # PACTRA_ATTEST_KEY=0x…
sudo cp ops/systemd/pactra-*.service /etc/systemd/system/
sudo systemctl daemon-reload
sudo systemctl enable --now pactra-meter pactra-attest
journalctl -u pactra-attest -n 40 --no-pager
```
...
```bash
sudo tee /etc/nginx/sites-available/pactra-demo-seller >/dev/null <<'EOF'
server {
    listen 80;
    server_name demo-seller.pactra.example;
    location ^~ /.well-known/acme-challenge/ { root /var/www/pactra; default_type "text/plain"; }
    location / { return 404; }
}
EOF
sudo ln -sf /etc/nginx/sites-available/pactra-demo-seller /etc/nginx/sites-enabled/pactra-demo-seller
sudo nginx -t && sudo systemctl reload nginx
sudo certbot certonly --webroot -w /var/www/pactra -d demo-seller.pactra.example

sudo cp ops/nginx/demo-seller.pactra.example.conf /etc/nginx/sites-available/pactra-demo-seller
sudo nginx -t && sudo systemctl reload nginx
sudo systemctl enable --now pactra-demo-seller    # the unit was copied with pactra-*.service above
curl -s https://demo-seller.pactra.example/health
```

It shares the `pactra_attest` rate-limit zone. `PACTRA_DEMO_PAYTO` in
...
both is the mistake that has already cost two outages here. Nothing Pactra
runs goes into pm2.
...
```bash
umask 077
cat > ~/pactra/.env.meter <<'EOF'
PACTRA_RPC=https://rpc.blockdaemon.testnet.arc.io
PACTRA_METER_CHUNK=10000
PACTRA_METER_MAX_BLOCKS=100000
PACTRA_METER_PACE_MS=100
EOF
```

`.env.attest` carries the same `PACTRA_RPC`. Both files are 0600 and neither
...
`packages/console/.env.local` on the box, read by Vite when `pactra-publish.sh`
rebuilds the bundle.

```bash
printf 'VITE_PRIVY_APP_ID=<the app id>\n' > ~/pactra/packages/console/.env.local
```
...
```bash
curl -s https://pactra.example/console/ | grep -o 'assets/index-[^"]*\.js' |
  head -1 | xargs -I{} curl -s --compressed "https://pactra.example/console/{}" |
  grep -c 'auth.privy.io'
```

## What this box does not run

**No daemon.** `pactra-meter`, `pactra-attest` and `pactra-demo-seller` are
the only Pactra units here, and none holds an operator key — the meter reads,
...
```bash
cd ~/pactra && git pull && ./ops/bin/pactra-publish.sh
```
...
```bash
PACTRA_ROOT=0x…  node packages/daemon/scripts/record-g2.mjs
PACTRA_ROOT=0x… PACTRA_DRILL=0x…  node packages/daemon/scripts/record-g3.mjs
```
...
`ConductRecord.RECORD_BASE` is a Solidity constant, so every record Pactra
writes carries `https://pactra.example/refusal/<id>` with the id the *chain*
...
`pactra-publish.sh` sets it to **`/api`** — the meter on this box's own origin,
proxied by nginx — so a redeploy needs nothing passed. `PACTRA_METER_URL`
overrides it for a meter somewhere else:

```bash
PACTRA_METER_URL=https://meter.example.com ./ops/bin/pactra-publish.sh
```

The meter already sends `access-control-allow-origin: *` — a record only its
owner can fetch is not a record.

`PACTRA_METER=http://127.0.0.1:8404 ops/bin/pactra-check.sh` also asks the
